...
*/
#define WF_WFDMA_HOST_DMA0_HOST_INT_STA_EXT_rx_done_int_sts_5_ADDR             \
	WF_WFDMA_HOST_DMA0_HOST_INT_STA_EXT_ADDR
#define WF_WFDMA_HOST_DMA0_HOST_INT_STA_EXT_rx_done_int_sts_5_MASK             \
	0x00200000 /* rx_done_int_sts_5[21] */
...
* 0: The size of descriptors is set to
16-byte
* 1: The size of descriptors is set to
32-byte
* FW_DWLD_Bypass_dmashdl[9] - (RW) No USE for (APSOC/PCIE)
* For firmware download packet, driver shold
...
(USB)
* For USB test_mode, user could set this bit
...
* CSR_TX_FCTRL_MODE[28] - (RW) 1: packet based TX flow control 0:
* DMAD based TX flow control
...
* CSR_TX_HALT_MODE[18] - (RW) 1'b1 : Halt TXDMAD pre-fetch/dispatch
* and ignore UDMA APB , idle indicator would exclude TX dispatch queue status
