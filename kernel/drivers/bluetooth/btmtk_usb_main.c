...
static void btmtk_usb_free_memory(void)
{
	...
	skb_queue_purge(&g_data->isoc_queue);
...
static void btmtk_usb_set_state(int new_state)
{
	static const char * const state_msg[] = {
		...
		"SUSPEND_PROBE", "SUSPEND_FW_DUMP", "RESUME_DISCONNECT", "RESUME_PROBE", "RESUME_FW_DUMP"
	};
...
void btmtk_usb_toggle_rst_pin(void)
{
	...
	BTUSB_WARN("%s: TCL bt reset begin!!!", __func__);
	...
	BTUSB_WARN("%s: TCL bt reset end!!!", __func__);
...
static void btmtk_usb_woble_free_setting(void)
{
	...
	if (g_data->woble_setting_len) {
		kfree(g_data->woble_setting);
		g_data->woble_setting_len = 0;
		g_data->woble_setting = NULL;
	}
	BTUSB_INFO("%s end", __func__);
}
		...
		if (search_result) {
			...
			do {
				if (search_result) {
					...
					if (!search_end || (search_end - search_result != CHAR2HEX_SIZE)) {
...
static int btmtk_usb_load_woble_setting(u8 *image, char *bin_name,
		struct device *dev, u32 *code_len, struct btmtk_usb_data *data)
{
	...
	if (err != 0) {
		kfree(image);
		image = NULL;
		BTUSB_ERR("%s: request_firmware function for woble setting  fail!! error code = %d", __func__, err);
		if (fw_entry)
	...
	if (fw_entry) {
		...
		BTUSB_ERR("%s: fw_entry is NULL request_firmware may fail!! error code = %d", __func__, err);
		return err;
	}
	if (image == NULL) {
		...
		const u8 *event, const int event_len)
{
	...
	btmtk_usb_add_to_hci_log(cmd, cmd_len, HCI_COMMAND_PKT);
...
static int btmtk_reboot_notify(struct notifier_block *notifier, ulong pm_event, void *unused)
{
	int ret = -1;
  BTUSB_INFO("%s: pm_event=%d", __func__,pm_event);
	switch (pm_event){
		case SYS_HALT:
		case SYS_POWER_OFF:
			ret = btmtk_usb_standby();
		break;
		default:
			ret = 0;
			break;
...
static int btmtk_usb_BT_init(void)
{
	...
	USB_MUTEX_UNLOCK();
#if (SUPPORT_UNIFY_WOBLE & SUPPORT_ANDROID)
	wake_lock_init(&g_data->woble_wlock, WAKE_LOCK_SUSPEND, "btevent_wakelock");
#endif
	spin_lock_init(&g_data->fwlog_lock);
	skb_queue_head_init(&g_data->fwlog_queue);
	spin_lock_init(&g_data->isoc_lock);
	skb_queue_head_init(&g_data->isoc_queue);

	g_data->reboot_notifier.notifier_call = btmtk_reboot_notify;
	register_reboot_notifier(&g_data->reboot_notifier);
...
static void btmtk_usb_BT_exit(void)
{
	...
	unregister_reboot_notifier(&g_data->reboot_notifier);
...
static int btmtk_usb_wait_until_event(u8 *wait_event, int wait_event_len, int total_timeout,
		int event_interval_timeout, int hci) /* hci event, otherwise use vend specic event */
{
	...
	do {
		...
		if (hci)
			btmtk_usb_push_data_to_metabuffer(g_data->io_buf, len, HCI_EVENT_PKT);
		else
			btmtk_usb_add_to_hci_log(g_data->io_buf, len, HCI_EVENT_PKT);

		/* check if receive complete event */
...
static int  btmtk_usb_reset_power_on(void)
{
	if (is_mt7668(g_data)) {
		...
		} else if (g_data->is_mt7668_dongle_state != BTMTK_USB_7668_DONGLE_STATE_POWER_ON) {
			BTUSB_ERR("%s: Incorrect state: %d", __func__, g_data->is_mt7668_dongle_state);
			return -1;
		}
		BTUSB_INFO("%s: Success", __func__);
	}
...
static void btmtk_usb_send_dummy_bulk_out_packet_7662(void)
{
	...
	memcpy(g_data->o_usb_buf, dummy_bulk_out_fuffer, 8);
...
static int btmtk_usb_load_rom_patch_7668(void)
{
	...
	do {
		...
		BTUSB_INFO("%s: patch_status %d", __func__, patch_status);
		...
		BTUSB_INFO("%s: no need to load rom patch", __func__);
			...
			BTUSB_INFO("%s: Download ROM patch directly!!!", __func__);
...
static int btmtk_usb_send_woble_suspend_cmd(void)
{
	...
	BTUSB_INFO("%s: set need_reset_stack = 1", __func__);
	need_reset_stack = 1;
...
static int btmtk_usb_send_assert_cmd_bulk(void)
{
	...
	ret = usb_bulk_msg(g_data->udev, usb_sndbulkpipe(g_data->udev, 2), g_data->o_usb_buf, sizeof(buf), &actual_length, 100);
	...
	if (g_data->woble_setting_len) {
		if (g_data->woble_setting_apcf_resume[0].length) {
			...
			for (i = 0; i < WOBLE_SETTING_COUNT; i++) {
				...
				BTUSB_INFO("%s: send radio on apcf %d", __func__, i);
...
resume_woble_done:
	if (ret) {
		...
		btmtk_usb_set_state(BTMTK_USB_STATE_WORKING);
		g_data->is_mt7668_dongle_state = BTMTK_USB_7668_DONGLE_STATE_POWER_ON;
		if (btmtk_woble_state == BTMTK_WOBLE_STATE_NEEDRESET_STACK)
			need_reset_stack = 1;
...
#define HCE_SYNC_CONN_COMPLETE 0x2C
		...
		if (intr_blocking_usb_warn < 10)
			BTUSB_INFO("%s: urb->status:%d, urb->length:%d, warn: %d", __func__, urb->status, urb->actual_length, intr_blocking_usb_warn);
	}
intr_resub:
	usb_mark_last_busy(g_data->udev);
	usb_anchor_urb(urb, &g_data->intr_in_anchor);
...
static void btmtk_usb_bulk_in_complete(struct urb *urb)
{
	...
	if (urb->actual_length > 4) {
		...
		if (buf[0] == 0x6f && buf[1] == 0xfc && len + 4 == urb->actual_length) {
			...
			if (state != BTMTK_USB_STATE_FW_DUMP && state != BTMTK_USB_STATE_RESUME_FW_DUMP) {
				...
				btmtk_woble_state = BTMTK_WOBLE_STATE_DUMPING;
			...
			if (skb_queue_len(&g_data->fwlog_queue) < FWLOG_ASSERT_QUEUE_COUNT) {
				...
				if (skb_fwlog == NULL) {
					BTUSB_ERR("%s: alloc_skb return 0, error", __func__);
					btmtk_woble_state = BTMTK_WOBLE_STATE_DUMPEND;
					btmtk_usb_toggle_rst_pin();
					picus_blocking_warn = 0;
	...
	if (urb->status == 0) {
		...
		if (bulk_in_blocking_usb_warn < 10)
			BTUSB_INFO("%s: urb->status:%d, warn:%d", __func__, urb->status, bulk_in_blocking_usb_warn);
	}
bulk_intr_resub:
...
static void btmtk_usb_isoc_complete(struct urb *urb)
{
	...
	/* Controller reply something to ISO_IN. */
		...
		if (urb->actual_length != 0 && sco_handle > 0) {
			...
			skb_queue_tail(&g_data->isoc_queue, skb_isoc);
			ISOC_SPIN_UNLOCK(flags);
			wake_up(&BT_sco_wq);
			wake_up_interruptible(&inq_isoc);
		}
	} else {
		BTUSB_INFO("%s ERROR! status:%d", __func__, urb->status);
	}
...
static ssize_t btmtk_usb_fops_read(struct file *file, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (state == BTMTK_USB_STATE_FW_DUMP || state == BTMTK_USB_STATE_RESUME_FW_DUMP) {
		/* do nothing */
	} else if (state == BTMTK_USB_STATE_SUSPEND_FW_DUMP) {
		...
		BTUSB_INFO("%s: Reset BT stack", __func__);
...
static int btmtk_usb_fops_close(struct inode *inode, struct file *file)
{
	...
	FOPS_MUTEX_UNLOCK();
	 ...
	 /*[MTK PATCH begin] [WCNCR00172083] Delete unnecessary power_on flow and unnecessary stack_restart*/
	 need_reset_stack = 0;
	 /*[MTK PATCH end] [WCNCR00172083] Delete unnecessary power_on flow and unnecessary stack_restart*/
		...
		size_t count, loff_t *f_pos)
{
	...
	btmtk_usb_set_isoc_interface(false);
	if (g_data->isoc_urb_submitted == 0)
		goto OUT;
		...
		size_t count, loff_t *f_pos)
{
	...
	/* means the buffer is empty */
	while (skb_queue_len(&g_data->isoc_queue) == 0) {
		/* If nonblocking mode, return directly O_NONBLOCK is specified during open() */
		if (file->f_flags & O_NONBLOCK) {
			/* BTUSB_DBG("Non-blocking btmtk_usb_fops_read()"); */
			retval = -EAGAIN;
		...
		wait_event(BT_sco_wq, skb_queue_len(&g_data->isoc_queue) > 0);
	}

	if (skb_queue_len(&g_data->isoc_queue) > 0) {
		u32 remain = 0;
		u32 shift = 0;
		u8 real_num = 0;
		u8 i = 1;
		...
		ISOC_SPIN_LOCK(flags);
		skb = skb_dequeue(&g_data->isoc_queue);
		ISOC_SPIN_UNLOCK(flags);
		if (skb == NULL) {
		...
		if (count < skb->len - real_num * HCI_SCO_HDR_SIZE) {
			...
			if (new_skb != NULL) {
				...
				new_skb->len = remain;
				ISOC_SPIN_LOCK(flags);
				skb_queue_head(&g_data->isoc_queue, new_skb);
				ISOC_SPIN_UNLOCK(flags);
			} else {
				BTUSB_WARN("sbk is NULL");
				goto OUT;
...
static int btmtk_usb_fops_sco_close(struct inode *inode, struct file *file)
{
	...
	btmtk_usb_set_isoc_interface(true);
...
/* [MTK PATCH] [WCNCR00172083] Delete unnecessary power_on flow and unnecessary stack_restart
	USB_MUTEX_LOCK();
	state = btmtk_usb_get_state();
	switch (state) {
...
*/
	if (is_support_unify_woble(g_data)) {
		...
		btmtk_usb_load_woble_setting(g_data->woble_setting,
			g_data->woble_setting_file_name,
			&g_data->udev->dev,
	...
	if (need_reset_stack) {
		/** After toggle reset, BD may write hci reset or some cmd without call fops_close
		 *  We should power on than BD can set hci cmd, otherwise controller may have issue.
		 */
		 /*[MTK PATCH begin] [WCNCR00172083] Delete unnecessary power_on flow and unnecessary stack_restart*/
		//btmtk_usb_send_init_cmds();
		if (is_support_unify_woble(g_data))
			btmtk_usb_send_init_cmds();
		/*[MTK PATCH end] [WCNCR00172083] Delete unnecessary power_on flow and unnecessary stack_restart*/
				
		if (is_support_unify_woble(g_data)) {
			BTUSB_INFO("%s: btmtk_woble_state %d", __func__, btmtk_woble_state);
			if (btmtk_woble_state == BTMTK_WOBLE_STATE_DUMPEND) { /* BTMTK_USB_STATE_SUSPEND_FW_DUMP */
				btmtk_usb_send_hci_reset_cmd();
				btmtk_usb_send_apcf_reserved();
				btmtk_usb_send_get_vendor_cap();
				btmtk_usb_unify_woble_suspend(g_data);
				/* Locked by suspend issued WoBLE entering failed case */
				btmtk_usb_woble_wake_unlock(g_data);
				btmtk_woble_state = BTMTK_WOBLE_STATE_NEEDRESET_STACK;
				need_reset_stack = 0;
			}
		}
		wake_up_interruptible(&inq);
...
static void btmtk_usb_disconnect(struct usb_interface *intf)
{
	...
	if (state == BTMTK_USB_STATE_SUSPEND || state == BTMTK_USB_STATE_SUSPEND_DISCONNECT) {
		...
		BTUSB_WARN("%s: state=%d disc hapens when driver is in resume, should stay in resume state later!",
				__func__, state);
