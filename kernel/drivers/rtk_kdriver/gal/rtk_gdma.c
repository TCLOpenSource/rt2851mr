...
#define MAX_NUM_DMA_BUF_GDMA_FENCE  2048 
dma_buf_gdma_lock_resource gdma_buf_gdma_lock[MAX_NUM_DMA_BUF_GDMA_FENCE];
static DEFINE_SPINLOCK(dma_buf_gdma_fence_lock);
static DEFINE_MUTEX(dma_buf_gdma_data_lock);
GDMA_PIC_DATA *gdma_fence_cur_pic = NULL;
GDMA_PIC_DATA *gdma_fence_pre_pic = NULL;
#if 1
...
static int gdma_ioctl_create_fence_get_fd(struct dma_fence *obj,
                                   unsigned int *ret_fd,struct sync_file **sync_file)
{
	int fd = 0;
	int err;
    ...
    *ret_fd = fd;
    ...
    //printk(KERN_EMERG"func=%s line=%d (*sync_file)=%x\n",__FUNCTION__,__LINE__,(*sync_file));
    //if(!(*sync_file)){
    //if(1){
        local_sync_file = sync_file_create(obj);
        *sync_file  = local_sync_file;
    //}else{
    //    local_sync_file = *sync_file;
    //}
    //printk(KERN_EMERG"func=%s line=%d local_sync_file=%x\n",__FUNCTION__,__LINE__,local_sync_file);
    //printk(KERN_EMERG"func=%s line=%d (*sync_file)=%x\n",__FUNCTION__,__LINE__,(*sync_file));
	//dma_fence_put(obj);
	if (!local_sync_file) {
		err = -ENOMEM;
...
err:
	put_unused_fd(fd);
    printk(KERN_EMERG"func=%s line=%d no local_sync_file!!\n",__FUNCTION__,__LINE__);
	return err;
}
...
//RTK mark for netfix
    ...
    case GDMA_GET_GDMA_FEND_INFO:
        {
            GDMA_FENCE_INFO gdma_fence_info_data;
            
            if(GDMA_SUCCESS == GDMA_GetFenceData(&gdma_fence_info_data)){
            if (copy_to_user((void __user *)arg, &gdma_fence_info_data, sizeof(GDMA_FENCE_INFO))) {
...
#endif
memset(gdma_buf_gdma_lock, 0, MAX_NUM_DMA_BUF_GDMA_FENCE * sizeof(dma_buf_gdma_lock_resource));
	return 0;
}
...
static void GDMA_OSDReg_Set(GDMA_DISPLAY_PLANE disPlane)
{
	...
	*/
    ...
    if(enable_osd1osd3mixerorder){
        mixer_layer_sel_reg.c0_sel = 2;//2:OSD2
        mixer_layer_sel_reg.c1_sel = 1;//2:OSD2
    }
#else
	...
	switch (data->type) {
		...
		case GDMA_DBG_CMD_OSD_GAL_AI_BLOCK:
		{
			...
			if(ai_num>=0 && ai_num <=5)
				GDMA_AI_SE_draw_block(w,h,ai_num,
							  ai_colors,ai_block,0) ;
			else
				GDMA_AI_SE_draw_block(w,h,5,
									  ai_colors,ai_block,0) ;
		}
			break;
...
int GDMA_ReceivePicture (PICTURE_LAYERS_OBJECT *data)
{
	...
	for (ilayer = 0; ilayer < data->layer_num; ilayer++) {
		...
		if (picObj->layer_used == 1) {
			...
			pic->workqueueDone = 0;
            if(disPlane == GDMA_PLANE_OSD1 && data->gdma_fence_enable){
                pic->gdma_buf_gdma_lock_index = data->gdma_fence_index;
                pic->gdma_buf_gdma_lock_index_enable = data->gdma_fence_enable;
                if(gdma_fence_cur_pic == NULL){
                    gdma_fence_cur_pic = pic;
                }else{
                    gdma_fence_pre_pic = gdma_fence_cur_pic;
                    gdma_fence_cur_pic = pic;                    
                }
            }else if(disPlane == GDMA_PLANE_OSD1 && gdma_fence_cur_pic != NULL){
              gdma_fence_pre_pic = gdma_fence_cur_pic;
              gdma_fence_cur_pic = NULL;
            }
            //RTK mark for netfix
			if(enable_osd1osd3mixerorder)
			{
			...
			*(char *)&pic->status = 0;
			...
			if(change_OSD_mode == GDMA_SPLIT_MODE_1){
				...
				if((picObj->width == 512 || picObj->width == 960) && picObj->height==2160){
					...
					//printk(KERN_EMERG"[%s:%d] g_esticker_info.cur_position %d\n", __func__, __LINE__, g_esticker_info.cur_position);
					...
					if(picObj->height < 1080)
						picObj->dst_height = 1080;
					else
						picObj->dst_height = picObj->height;
					win->winWH.width  = (picObj->width);
					if(picObj->format == VO_OSD_COLOR_FORMAT_ARGB8888_LITTLE 
						...
						win->pitch            = win->winWH.width*2;					
					if(picObj->width < 1920)
						picObj->dst_width = 1920;
					else
						picObj->dst_width = win->winWH.width;
                    
					//reset esticker position when exit esticker app
					if(g_esticker_info.cur_position == 5760)
						g_esticker_info.cur_position = 300;
					else if(g_esticker_info.cur_position == 2912)
						g_esticker_info.cur_position = 150;
	...
	for (idx = 0; idx < onlineMaxNum; idx++) {
		...
		//		gdma->pendingRPC[disPlane] = 1;
				...
				if(disPlane==GDMA_PLANE_OSD1)
					gdma_to_dump();
                if(disPlane == GDMA_PLANE_OSD1){
                    if(gdma_fence_pre_pic != NULL && gdma_fence_pre_pic->gdma_buf_gdma_lock_index_enable){
                        GDMA_SignalFenceData(gdma_fence_pre_pic->gdma_buf_gdma_lock_index);
                        gdma_fence_pre_pic->gdma_buf_gdma_lock_index_enable = 0;
                    }
                }
...
int GDMA_GetFenceData(GDMA_FENCE_INFO *data)
{
     int i,found=0;//,ret;
     static int pre_j=0;
     if(data == 0)
         return GDMA_FAIL;
mutex_lock(&dma_buf_gdma_data_lock);
     if(pre_j > (MAX_NUM_DMA_BUF_GDMA_FENCE-(MAX_NUM_DMA_BUF_GDMA_FENCE/10)))
         pre_j = 0;
 #if 0
     for(k=0;k<MAX_NUM_DMA_BUF_GDMA_FENCE;k++){
         if(gdma_buf_gdma_lock[k].created == 1)
         {
             ret = dma_fence_get_status(&gdma_buf_gdma_lock[k].gdma_fence);
             if(ret != 1)
                printk(KERN_EMERG"func=%s line=%d ret=%d != 1 num=%d\n",__FUNCTION__,__LINE__,ret,k);
         }
     }
 #endif
     for(i=pre_j;i<MAX_NUM_DMA_BUF_GDMA_FENCE;i++){
         if(gdma_buf_gdma_lock[i].used == 0){

             //if(gdma_buf_gdma_lock[i].created && (dma_fence_get_status(&gdma_buf_gdma_lock[i].gdma_fence) != 1))
             //{
             //printk(KERN_EMERG"func=%s line=%d dma_fence_get_status index=%d \n",__FUNCTION__,__LINE__,i);
             //    continue;
             //}
             found = 1;
             gdma_buf_gdma_lock[i].used = 1;
             gdma_ioctl_create_fence_get_fd(&gdma_buf_gdma_lock[i].gdma_fence,
                                            &gdma_buf_gdma_lock[i].gdma_fence_fd,
                                            &gdma_buf_gdma_lock[i].gdma_fence_sync_file);
            //dma_fence_enable_sw_signaling(&gdma_buf_gdma_lock[i].gdma_fence);
            data->gdma_fence_index = i;
            data->gdma_fence_fd = gdma_buf_gdma_lock[i].gdma_fence_fd;
            gdma_buf_gdma_lock[i].created = 1 ;
            //printk(KERN_EMERG"func=%s line=%d get index=%d data->gdma_fence_fd=%d\n",__FUNCTION__,__LINE__,i,data->gdma_fence_fd);
            //printk(KERN_EMERG"func=%s line=%d get index=%d current->pid=%d\n",__FUNCTION__,__LINE__,i,current->pid);
            break;
         }
     }
     if(found)
        pre_j = i;
mutex_unlock(&dma_buf_gdma_data_lock);
...
int GDMA_SignalFenceData(int gdma_fence_index)
{
//    int count=0;
    int ret=0;
    //printk(KERN_EMERG"func=%s line=%d release index=%d \n",__FUNCTION__,__LINE__,gdma_fence_index);
    mutex_lock(&dma_buf_gdma_data_lock);    
    if(gdma_fence_index >=0 && gdma_fence_index < MAX_NUM_DMA_BUF_GDMA_FENCE){
        if(gdma_buf_gdma_lock[gdma_fence_index].used){

            ret=dma_fence_signal(&gdma_buf_gdma_lock[gdma_fence_index].gdma_fence);
            if(ret<0)
                printk(KERN_EMERG"func=%s line=%d signal fence ret=%d \n",__FUNCTION__,__LINE__,ret);
#if 0
            while(1){
                
                ret = dma_fence_get_status(&gdma_buf_gdma_lock[gdma_fence_index].gdma_fence);
                printk(KERN_EMERG"func=%s line=%d get fence ret=%d != 1 \n",__FUNCTION__,__LINE__,ret);
                if(ret || count++ > 100)
                    break;
                gdma_usleep(10000); 
                ret=dma_fence_signal_locked(&gdma_buf_gdma_lock[gdma_fence_index].gdma_fence);
                printk(KERN_EMERG"func=%s line=%d signal fence ret=%d \n",__FUNCTION__,__LINE__,ret);
            }
#endif
            //put_unused_fd(gdma_buf_gdma_lock[gdma_fence_index].gdma_fence_fd);
            gdma_buf_gdma_lock[gdma_fence_index].used = 0;
            //printk(KERN_EMERG"func=%s line=%d release index=%d \n",__FUNCTION__,__LINE__,gdma_fence_index);
        }
    }
mutex_unlock(&dma_buf_gdma_data_lock);
...
int GDMA_ReceiveGraphicLayers(GRAPHIC_LAYERS_OBJECT *data)
{
	...
	*/
	...
	if(data->cookie == NONBLOCKING_COOKIE){
		...
		if (gdmaLayerNum > 0){
            sendGdmaLayerObj->gdma_fence_enable = data->comp_done;
            sendGdmaLayerObj->gdma_fence_index = data->wb_img_addr;
...
int GDMA_CaptureFramebuffer(KGAL_SURFACE_INFO_T *pDstSurfaceInfo)
{
	...
	if (GDMA_DBG_OSD_WI_1_get_colortype(rtd_inl(GDMA_DBG_OSD_WI_1_reg)) == VO_OSD_COLOR_FORMAT_ARGB8888 ||
		...
		pixelFormat = KADP_HAL_GAL_PIXEL_FORMAT_ARGB;
	} else {
