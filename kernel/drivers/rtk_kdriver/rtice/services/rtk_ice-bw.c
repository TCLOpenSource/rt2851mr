...
#ifdef SUPPORT_SCPU_PC
	dc1_mc_scpu_pc_ctrl_RBUS      reg_scpu_pc_ctrl;
#endif
	/*dcu*/
...
#ifdef SUPPORT_SCPU_PC
	/*scpu on DCU1*/
	reg_scpu_pc_ctrl.regValue = rtd_inl(dcu_addr(DC1_MC_SCPU_PC_CTRL_reg,FALSE));
	reg_scpu_pc_ctrl.pc_go = pc_go ? 1 : 0;
	rtd_outl(dcu_addr(DC1_MC_SCPU_PC_CTRL_reg,FALSE), reg_scpu_pc_ctrl.regValue);
#ifdef SUPPORT_DCU2 
	/*scpu on DCU2*/
	reg_scpu_pc_ctrl.regValue = rtd_inl(dcu_addr(DC1_MC_SCPU_PC_CTRL_reg,TRUE));
	reg_scpu_pc_ctrl.pc_go = pc_go ? 1 : 0;
	rtd_outl(dcu_addr(DC1_MC_SCPU_PC_CTRL_reg,TRUE), reg_scpu_pc_ctrl.regValue);
#endif    
#endif
	return 0;
}
...
static int effi_measure_en(char fen,int fdcu2)
{
	...
	if(reg_eff_meas_ctrl.eff_meas_en != fen ||
		...
		if(g_bwCheck.m_measPeriod)
			reg_eff_meas_ctrl.meas_period = g_bwCheck.m_measPeriod;
		rtd_outl(dcu_addr(DC1_MC_EFF_MEAS_CTRL_reg,fdcu2),\
						reg_eff_meas_ctrl.regValue);
		//reg_eff_meas_ctrl.regValue = rtd_inl(EFF_MEAS_CTRL_reg);
...
static int check_result(UINT32*pRet,unsigned int len,int fdcu2)
{
	...
	dc1_mc_scpu_pc_tota_mon_num_RBUS       reg_scpu_pc_tota_mon_num;
	dc1_mc_scpu_pc_ack_num_RBUS            reg_scpu_pc_ack_num;
	UINT32 FreqS;
#endif
    ...
    *ptotal_idle2 = 0;
	...
	/*efficient extension info(conflict ,w2r,r2w count)*/
	eff2[3] = DC1_MC_BG_UNBALANCE_CNT_get_bg_unbalance_cnt(\
				rtd_inl(dcu_addr(DC1_MC_BG_UNBALANCE_CNT_reg,TRUE)));//bg_unbalance_cnt
    /*small range ratio*/
    ...
    dividend = (u64)2*reg_read_cmd.rd_cnt - (u64)reg_read_chop_cmd.rd_chop_cnt - \
            (u64)mc_acc_rd_cnt_reg.parst_acc_rd_cnt;
    dividend += (u64)reg_write_cmd.wr_cnt - (u64)reg_write_chop_cmd.wr_chop_cnt - \
            (u64)mc_acc_wr_cnt_reg.parst_acc_wr_cnt;
    divisor = (u64)reg_read_cmd.rd_cnt + (u64)reg_write_cmd.wr_cnt;
	...
	if(len > 14 && (len - 14) % 3 == 0){
        ...
        if(!fdcu2){
    		...
    		dividend = (u64)1600 * (u64)ipbw[0] * (u64)FreqH;
    		RTICE_DEBUG("%llu = %u * %u * %u\r\n",dividend,1600,ipbw[0],FreqH);
    		dividend =  divisor == 0 ? 0 :div64_u64(dividend,divisor);
    		ip[0] = dividend;
    		RTICE_DEBUG("%d = %llu / %llu \r\n",ip[0],dividend,divisor);
    		dividend = (u64)1600 * (u64)ipbw[1] * (u64)FreqH;
    		RTICE_DEBUG("%llu = %u * %u * %u\r\n",dividend,1600,ipbw[1],FreqH);
    		dividend =  divisor == 0 ? 0 :div64_u64(dividend,divisor);
    		ip[1] = dividend;
    		RTICE_DEBUG("%d = %llu / %llu \r\n",ip[1],dividend,divisor);
    		dividend = (u64)1600 * (u64)ipbw[2] * (u64)FreqH;
    		RTICE_DEBUG("%llu = %u * %u * %u\r\n",dividend,1600,ipbw[2],FreqH);
    		dividend =  divisor == 0 ? 0 :div64_u64(dividend,divisor);
    		RTICE_DEBUG("%d = %llu / %llu \r\n",ip[2],dividend,divisor);
    		ip[2] = dividend;
    		/*Max Latency*/
	...
	reg_scpu_pc_tota_mon_num.regValue = \
		rtd_inl(DC1_MC_SCPU_PC_TOTA_MON_NUM_reg);
	reg_scpu_pc_ack_num.regValue = rtd_inl(DC1_MC_SCPU_PC_TOTA_ACK_NUM_reg);
	N = (rtd_inl(0xb8008028) >> 8) & 0xff;
	F = (rtd_inl(0xb8008024) >> 16) & 0x3ff;
	FreqS = 2700 * ((N + 3) * 2048 + F) / 2048  / 100/2;
	/*bw = 16 * reg_scpu_pc_ack_num.ack_num *
	 *   FreqS / reg_scpu_pc_tota_mon_num.tota_mon_num*/
	dividend = FreqS;
	...
	RTICE_DEBUG("[%s:%d]%llu = dividend / %llu\r\n", __func__,__LINE__,dividend,divisor);
	*pscup_bw  = dividend;
#ifdef SUPPORT_DCU2
	/*scpu bw of dcu2*/
	N = (rtd_inl(0xb8004028) >> 8) & 0xff;
	F = (rtd_inl(0xb8004024) >> 16) & 0x3ff;
	FreqS = 2700 * ((N + 3) * 2048 + F) / 2048  / 100 /2;
	reg_scpu_pc_tota_mon_num.regValue = \
		rtd_inl(dcu_addr(DC1_MC_SCPU_PC_TOTA_MON_NUM_reg,TRUE));
	reg_scpu_pc_ack_num.regValue = rtd_inl(dcu_addr(DC1_MC_SCPU_PC_TOTA_ACK_NUM_reg,TRUE));
	dividend = FreqS;
	dividend = dividend * (u64)1600 * (u64)reg_scpu_pc_ack_num.ack_num;
	divisor = reg_scpu_pc_tota_mon_num.tota_mon_num ;
	if(divisor == 0)
...
static int bw_check_thread(void*data)
{
	...
	for(i = 0;!kthread_should_stop() &&  \
		...
		while(!kthread_should_stop() && pCheck->f_run &&  \
			...
			RTICE_DEBUG("[%s:%d]Thread Status:Thread=0x%08x;f_run=%d;f_stop=%d;f_stoped=%d\r\n", \
				__func__,__LINE__, (UINT32)g_bwCheck.p_thread,g_bwCheck.f_run, \
				g_bwCheck.f_stop,g_bwCheck.f_stoped);
			if(g_bwCheck.f_stop == TRUE){
