...
#include "rtk_otp_ecc_algorithm.h"
...
int EnHammingCode1024(char* orig_data, unsigned int* ecc_data)
{
    ...
    if(orig_data == NULL) {
        OTP_WARNING("[Error] Empty data ponter!\n");
        return -1;
    }

    if(ecc_data == NULL) {
        OTP_WARNING("[Error] Empty ecc data ponter!\n");
        return -2;
    }
    ...
    if(syn <= (THE_NUM_CHECK_1024_BITS + THE_NUM_DATA_1024_BITS)) {
        if (syn) {
            ...
            if((get_bit_num_idx & 0x01) != ((ecc_data >> THE_NUM_CHECK_1024_BITS) & 0x01)) {
                ...
                OTP_WARNING("[Info] repaired ok\n");
            } else {
                OTP_WARNING("[Error] unrepaired\n");
                ret = ECC_CHECK_RESULT_FAIL_BUT_NOT_FIXED;
            }
        } else {
            OTP_WARNING("[Info] No error for data\n");
            ret = ECC_CHECK_RESULT_SUCC;
        }
    } else {
        OTP_WARNING("[Error] Get Error syndrome\n");
        ret = ECC_CHECK_RESULT_FAIL_BUT_NOT_FIXED;
    }
                  ...
                  unsigned int* ecc_data, unsigned int *ecc_data_bits_len)
{
    ...
    *ecc_data_bits_len = get_ecc_check_bits_len(orig_data_len);
    if(*ecc_data_bits_len == 0) {
        OTP_WARNING("[Error] Can't get valid Ecc check bits len\n");
        return -1;
    }
...
int do_ecc_check_util(ECC_CHECK_REGION *check_region)
{
    ...
    OTP_DBG("%s, %d, check_unit_data_len == %d\n", check_unit_data_len);
    ...
    for(i = 0; i < loops; i++) {
        ...
        if(read_ecc_bits(check_ecc_offset, check_region->ecc_unit_bit_num, &ecc_val) != 0) {
            OTP_WARNING("read ecc bits failed ,ecc offset= %08x\n", check_ecc_offset);
            ret = -1;
            break;
        }
    ...
    ECC_INPUT_DATA_PARAM *params, unsigned char **output)
{
    ...
    if(otp_read_region_byte(params->data_ofst, buffer, params->data_len) != 0) {
        OTP_WARNING("Read data failed, ofst = %08x, len == %d\n",
                         params->data_ofst, params->data_len);
        goto error;
    }
    if(check_data_burn(buffer, params->data_len) != 0) {
        OTP_WARNING("Data not burn, ofst = %08x, len == %d\n",
                         params->data_ofst, params->data_len);
        goto error;
    }
    ...
    if(do_ecc_check_util(&check_region) != ECC_CHECK_RESULT_SUCC) {
        OTP_WARNING("Data check failed, ofst = %08x, len == %d\n", params->data_ofst, params->data_len);
        goto error;
    }
    *output = buffer;
    return 0;
                                 ...
                                 unsigned int **output)
{
    ...
    return do_get_data_region_with_ecc_check(&param, (unsigned char **)output);
}
