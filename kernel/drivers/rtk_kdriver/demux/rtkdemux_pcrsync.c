	...
	if ( (RCD == -1) )
	{
		...
		*/
			...
			)
		{
			...
			dmx_notice("ch[%d] RCD  deliver:check-A/V:%d/%d, pcr:%lld, pts-A/V: %lld/%lld, thresthold-A/V:%d/%ld, cur/timeout: %lld/%lld\n",
...
void PCRSYNC_UpdateDemuxPTS(DEMUX_PCRSYNC_T *pPcrProcesser, bool bVideo, int64_t pts)
{
	if (bVideo) {
		...
		if (pPcrProcesser->startAudioPts == -1) {
			dmx_crit("[dtv] ch[%d] deliver first audio pts = %lld, time = %lld\n", (int)pPcrProcesser->tunerHandle, pts, CLOCK_GetAVSyncPTS(pPcrProcesser->pRefClock));
