...
{
    DC_MT_DESC mt_desc;
    unsigned int mem_size, mem_addr,i;
    unsigned long total_ddr_size = 0x80000000;
    ...
    if(total_ddr_size != 0x80000000){
        pr_err("[%s] DDR total size(0x%lx) != 2G,only merlin5 2G platform need to set boundaries\n", __FUNCTION__, total_ddr_size);
        return;
    }
    ...
    for(i = 0; i < 2; i++){
        ...
        //pr_err("[%s] skip mem_addr2=0x%x mem_size2=0x%x\n", __FUNCTION__, mem_addr, mem_size);
        if (mem_size==0 ||mem_addr==0)
        {
            pr_err("[%s] skip CARVEDOUT_DDR_BOUNDARY[%d] setting ......\n",__FUNCTION__, i);
            continue;
        }
