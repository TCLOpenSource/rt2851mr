...
INT32 Tp_Locker_SetLockFunction(char myMutexName[TP_MUTEX_NAME_LEN], const char * myFunName)
{
        ...
        if (p_this && p_funCallStack_this) {
                ...
                //memcpy(&p_this->name, &myMutexName, sizeof(p_this->name));
                //memcpy(&p_funCallStack_this->funName, &myFunName, sizeof(p_funCallStack_this->funName));
                snprintf(p_this->name, sizeof(p_this->name), "%s", myMutexName);
                snprintf(p_funCallStack_this->funName, sizeof(p_funCallStack_this->funName), "%s", myFunName);
                p_funCallStack_this->lock = 1;
        } else {
                if (!p_this)
                        TP_ERROR("Can not allocate memory for TP_DRV_MUTEXLOCK_T (structure size=%ld)\n", (unsigned long)sizeof(TP_DRV_MUTEXLOCK_T));
                if (!p_funCallStack_this)
                        TP_ERROR("Can not allocate memory for TP_DRV_MUTEXFUNNAME_T (structure size=%ld)\n", (unsigned long)sizeof(TP_DRV_MUTEXFUNNAME_T));
