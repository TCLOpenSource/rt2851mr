...
unsigned char CS602_Data_BankA_50inch_U2[49] =
{
	0x00, 0x48, 0x1B, 0xA4, 0x00, 0x27, 0x29, 0xFC, 0x66, 0x7B, 0x2E, 0x2A, 0x00, 0x10, 0x10, 0x09, 0x09,
	0x3E, 0x43, 0xD4, 0x35, 0x33, 0x06, 0x2E, 0x02, 0x7C, 0x26, 0x31, 0xFD, 0x1E, 0x11, 0x7A, 0x14,
	0xA0, 0xEA, 0x03, 0xB0, 0x21, 0x1C, 0xE1, 0xCE, 0x98, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};	

unsigned char CS602_Data_BankA_55inch_D12[49] =
{
	0x00, 0x48, 0x19, 0xA4, 0x00, 0x03, 0x23, 0xF8, 0x66, 0x7B, 0x2B, 0x28, 0x00, 0x10, 0x10, 0x07, 0x07,
	0x3E, 0x63, 0xC9, 0x32, 0x32, 0xCC, 0x29, 0xE2, 0x33, 0x21, 0x81, 0xBD, 0x1A, 0x01, 0x33, 0x0F,
	0xE0, 0xA2, 0x01, 0xC0, 0x0D, 0x1A, 0x11, 0x8F, 0x95, 0x82, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

unsigned char CS602_Data_BankA_65inch_D02_H[49] =
{
	0x00, 0x48, 0x19, 0x92, 0x00, 0x2B, 0x23, 0xFC, 0x66, 0x7B, 0x2E, 0x2C, 0x00, 0x10, 0x10, 0x0B, 0x0B,
	0x3E, 0x53, 0xE0, 0x35, 0xC2, 0xFC, 0x2C, 0x02, 0x31, 0x20, 0x81, 0xAA, 0x18, 0x11, 0x32, 0x0D,
	0x00, 0x5B, 0x01, 0x50, 0x10, 0x19, 0xB2, 0xDB, 0xDE, 0x4F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
unsigned char CS602_Data_BankA_55inch_D09_1[49] =
{
	0x00, 0x48, 0x19, 0xA4, 0x00, 0x27, 0x23, 0xFC, 0x66, 0x7B, 0x2B, 0x28, 0x00, 0x14, 0x14, 0x07, 0x07,
	0x3E, 0x23, 0xBC, 0x31, 0x82, 0xCF, 0x2A, 0x52, 0x36, 0x20, 0x01, 0xC8, 0x19, 0x21, 0x37, 0x10,
	0xA0, 0xBE, 0x04, 0x30, 0x0E, 0x1A, 0x51, 0xA5, 0xE6, 0xAA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static int read_cs602_data_from_reg(void)
{
	int ret,i,sum = 0;
	...
	if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14) && (pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 228))||
		((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14) && (pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 229))||
		((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14) && (pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 230))||
		((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14) && (pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231)))
	{
		...
		ret = i2c_master_send_ex(1,CS602_DEV_ADDR,&temp_cs602[0],43);
...
//this funciton is for tconless panel with cs602, since cs602 has not e2prom when lose electricity, it needs to rewrite data by i2c 
//add by zhangshu
void tconless_cs602_reset(void)
{
	int ret,i;
	if(read_cs602_data_from_reg() != -1)
	{
	...
	else
	{
		for(i = 0; i < 3; i++)
                {  
			if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 230))
			{
				ret = i2c_master_send_ex(1,CS602_DEV_ADDR,&CS602_Data_BankA_65inch_D02_H[0],CS602_DATA_NUM+1);
			}
			else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 229))
			{
				ret = i2c_master_send_ex(1,CS602_DEV_ADDR,&CS602_Data_BankA_55inch_D12[0],CS602_DATA_NUM+1);
			} 		
			else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 228))
			{
				ret = i2c_master_send_ex(1,CS602_DEV_ADDR,&CS602_Data_BankA_50inch_U2[0],CS602_DATA_NUM+1);
			}
			else if((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14)&&(pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231))
			{
				ret = i2c_master_send_ex(1,CS602_DEV_ADDR,&CS602_Data_BankA_55inch_D09_1[0],CS602_DATA_NUM+1);
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] this panel is not tconless panel with cs602\n",__FUNCTION__);
                        	return;
			}
		
			if(ret < 0)
			{		
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c cs602 write fail\n",__FUNCTION__);
			}
			else
			{
				rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] i2c cs602 write ok\n",__FUNCTION__);
                        	return;
			}
                }  
	}
...
void tconless_CS602_reserve(void)
{
	int ret;
	int i;
	...
	if(((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14) && (pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 228))||
		...
		((pPanelCOnfigParameter->iCONFIG_PANEL_TYPE == 14) && (pPanelCOnfigParameter->iCONFIG_PANEL_CUSTOM_INDEX == 231)))
	{
		ret = i2c_master_recv_ex(1, CS602_DEV_ADDR, &sub_addr, 1, &cs602_reg[0], 42);
	...
	for(i=0; i<42; i++) //for debug
		rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]zhangshu [%x]\n",__FUNCTION__,cs602_reg[i]);
