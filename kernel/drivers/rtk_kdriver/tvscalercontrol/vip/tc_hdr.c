...
//1.
	...
	//3.
		...
		if (status == 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_C_LUT write error, return\n");
			return;
		}
		if ( tc_tchdr_c_lut_ctrl1.c_lut_rw_addr != i + 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_C_LUT write error : c_lut_rw_addr, return\n");
			return;
		}
...
//3.
		...
		if (status_read_en == 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_C_LUT read error, return\n");
			return;
		}
		if ( tc_tchdr_c_lut_ctrl1.c_lut_rw_addr != i + 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_C_LUT write error : c_lut_rw_addr, return\n");
...
//3.
		...
		if (status_read_en == 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_I_LUT read error, return\n");
			return;
		}
		if ( tc_tchdr_i_lut_ctrl1.i_lut_rw_addr != i + 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_I_LUT write error : i_lut_rw_addr, return\n");
			return;
		}
...
//1.
	...
	//3.
		...
		if (status == 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_I_LUT write error, return\n");
			return;
		}
		if ( tc_tchdr_i_lut_ctrl1.i_lut_rw_addr != i + 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_I_LUT write error : i_lut_rw_addr, return\n");
...
//1.
	...
	//3.
		...
		if (status == 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_S_LUT write error, return\n");
			return;
		}
		if ( tc_tchdr_s_lut_ctrl1.s_lut_rw_addr != i + 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_S_LUT write error : s_lut_rw_addr, return\n");
			return;
		}
...
//3.
		...
		if (status_read_en == 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_S_LUT read error, return\n");
			return;
		}
		if ( tc_tchdr_s_lut_ctrl1.s_lut_rw_addr != i + 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_S_LUT write error : s_lut_rw_addr, return\n");
...
//2.
	...
	if (status_write_enable == 1) {
		rtd_printk(KERN_EMERG, TAG_NAME, " TC_P_LUT write error, return\n");
		return -2;
	}
	...
	if ( tc_tchdr_p_lut_ctrl2.p_lut_rw_addr != i + 1) {
		rtd_printk(KERN_EMERG, TAG_NAME, " TC_P_LUT write error : p_lut_rw_addr, return\n");
		return -3;
	}
}//end for
//5
...
//4.
	...
	if (status_read_enable == 1) {
		rtd_printk(KERN_EMERG, TAG_NAME, " TC_P_LUT read error, return\n");
		return -2;
	}
...
//3.
		...
		if (status_read_en == 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_D_LUT read error, return\n");
			return;
		}
		if ( tc_tchdr_d_lut_ctrl1.d_lut_rw_addr != i + 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_D_LUT write error : d_lut_rw_addr, return\n");
			return;
		}
...
//1.
	...
	//3.
		...
		if ( tc_tchdr_d_lut_ctrl1.d_lut_rw_addr != i + 1) {
			rtd_printk(KERN_EMERG, TAG_NAME, " TC_S_LUT write error : d_lut_rw_addr, return\n");
			return;
		}
...
void drvif_fwif_color_set_TC_v130_Compute_muA_muB(SL_HDR1_metadata *sl_hdr1_metadata, unsigned short *muA, unsigned short *muB)
{
    ...
    if (!sl_hdr1_metadata->payloadMode) {
		...
		if(valNu >= 16)
		{
			rtd_printk(KERN_EMERG, TAG_NAME, "Error: muA_709/muA_2020/muB_709/muB_2020 of size 16 use index value greater than 16\n");
...
void drvif_fwif_color_set_TC_v130_Write_C_LUT(unsigned short *C_LUT)
{
	...
	if(pTC_HDR_CTRL->C_LUT_DMA.phy_addr_align==0	||	pTC_HDR_CTRL->C_LUT_DMA.pVir_addr_align==NULL){
		rtd_printk(KERN_EMERG, TAG_NAME, "C_LUT_DMA.phy_addr_align = %d, C_LUT_DMA.pVir_addr_align=%p\n",pTC_HDR_CTRL->C_LUT_DMA.phy_addr_align,pTC_HDR_CTRL->C_LUT_DMA.pVir_addr_align);
		return;
	}
...
void drvif_fwif_color_set_TC_v130_Write_P_LUT(unsigned short *P_LUT)
{
	...
	if(pTC_HDR_CTRL->P_LUT_DMA.phy_addr_align==0	||	pTC_HDR_CTRL->P_LUT_DMA.pVir_addr_align==NULL){
		rtd_printk(KERN_EMERG, TAG_NAME, "P_LUT_DMA.phy_addr_align = %d, P_LUT_DMA.pVir_addr_align=%p\n",pTC_HDR_CTRL->P_LUT_DMA.phy_addr_align,pTC_HDR_CTRL->P_LUT_DMA.pVir_addr_align);
		return;
	}
...
//	DRV_TCHDR_D_lut_step_thl data;
	...
	switch (curve) {
	    ...
	    case PQ:
	      if (proc_mode == SDR2MDR) {
	        switch (MaxTargetDisplayLum)
	        {
		     ...
		     rtd_printk(KERN_EMERG, TAG_NAME, "TC : Fill_D_LUT : Can only handle peak luminance from 100 to 2000 nits\n !!!\n");
		...
		rtd_printk(KERN_EMERG, TAG_NAME, "TC : Fill_D_LUT : curve not handled\n !!!\n");
	      return;
  }
...
void drvif_fwif_color_set_TC_v130_Write_D_LUT(unsigned short *D_LUT)
{
	...
	if(pTC_HDR_CTRL->D_LUT_DMA.phy_addr_align==0	||	pTC_HDR_CTRL->D_LUT_DMA.pVir_addr_align==NULL){
		rtd_printk(KERN_EMERG, TAG_NAME, "D_LUT_DMA.phy_addr_align = %d, D_LUT_DMA.pVir_addr_align=%p\n",pTC_HDR_CTRL->D_LUT_DMA.phy_addr_align,pTC_HDR_CTRL->D_LUT_DMA.pVir_addr_align);
		return;
	}
...
void drvif_fwif_color_set_TC_v130_Write_I_LUT(unsigned char src_idx,unsigned char I_lut_idx)
{
	...
	if(pTC_HDR_CTRL->I_LUT_DMA.phy_addr_align==0	||	pTC_HDR_CTRL->I_LUT_DMA.pVir_addr_align==NULL){
		rtd_printk(KERN_EMERG, TAG_NAME, "I_LUT_DMA.phy_addr_align = %d, I_LUT_DMA.pVir_addr_align=%p\n",pTC_HDR_CTRL->I_LUT_DMA.phy_addr_align,pTC_HDR_CTRL->I_LUT_DMA.pVir_addr_align);
		return;
	}
...
void drvif_fwif_color_set_TC_v130_Write_S_LUT(unsigned char src_idx,unsigned char S_lut_idx)
{
	...
	if(pTC_HDR_CTRL->S_LUT_DMA.phy_addr_align==0	||	pTC_HDR_CTRL->S_LUT_DMA.pVir_addr_align==NULL){
		rtd_printk(KERN_EMERG, TAG_NAME, "S_LUT_DMA.phy_addr_align = %d, S_LUT_DMA.pVir_addr_align=%p\n",pTC_HDR_CTRL->S_LUT_DMA.phy_addr_align,pTC_HDR_CTRL->S_LUT_DMA.pVir_addr_align);
		return;
	}
...
void drvif_fwif_color_set_TC_v130_Compute_P_LUT(int proc_mode, SL_HDR1_metadata *sl_hdr1_metadata, int TargetDisplayLuminance, unsigned short *P_LUT)
{
  ...
  if (!sl_hdr1_metadata->payloadMode) { // payloadMode = 0 (Parameter-based)
    ...
    if ((proc_mode == SDR2MDR && TargetDisplayLuminance != sl_hdr1_metadata->hdrMasterDisplayMaxLuminance) ||
      ...
      rtd_printk(KERN_EMERG, TAG_NAME, "Display Adaptation can't work with this payloadMode\n");
      return;
    }
    else {
      for (i = 0; i < 32; i++) {
...
)
{
	...
	rtd_printk(KERN_INFO, TAG_NAME, "TC ini, get phy addr, C_ addr = %x, I_ addr = %x, S_ addr = %x, P_ addr = %x, D_ addr = %x,\n",
	...
	rtd_printk(KERN_INFO, TAG_NAME, "TC ini, get Vir addr, C_ addr = %p, I_ addr = %p, S_ addr = %p, P_ addr = %p, D_ addr = %p,\n",
...
char drvif_color_set_TC_Write_D_LUT_By_DMA(unsigned short *d_lut_coef_ptr, unsigned int addr, unsigned int* vir_addr)
{
    ...
    //rtd_printk(KERN_INFO, TAG_NAME, "drvif_color_set_TC_Write_D_LUT_By_DMA :  tc_d_lut : phys %d , virtaddr %p \n",addr, vir_addr );

    if ((d_lut_coef_ptr == NULL)  || (addr == 0) || (vir_addr == NULL)) {
           rtd_printk(KERN_EMERG, TAG_NAME, "drvif_TC_Set_By_DMA, table NULL, d_lut_coef_ptr=%p, addr = %d, vi_addr = %p\n",
...
char drvif_color_set_TC_Write_C_LUT_By_DMA(unsigned short *c_lut_coef_ptr, unsigned int addr, unsigned int* vir_addr)
{
    ...
    //rtd_printk(KERN_INFO, TAG_NAME, "drvif_color_set_TC_Write_C_LUT_By_DMA :  tc_c_lut : phys %d , virtaddr %p \n",addr, vir_addr );

    if ((c_lut_coef_ptr == NULL)  || (addr == 0) || (vir_addr == NULL)) {
           rtd_printk(KERN_EMERG, TAG_NAME, "drvif_TC_Set_By_DMA, table NULL, c_lut_coef_ptr=%p, addr = %d, vi_addr = %p\n",
...
char drvif_color_set_TC_Write_P_LUT_By_DMA(unsigned short *p_lut_coef_ptr, unsigned int addr, unsigned int* vir_addr)
{
    ...
    //rtd_printk(KERN_INFO, TAG_NAME, "drvif_color_set_TC_Write_P_LUT_By_DMA :  tc_p_lut : phys %d , virtaddr %p \n",addr, vir_addr );

    if ((p_lut_coef_ptr == NULL)  || (addr == 0) || (vir_addr == NULL)) {
           rtd_printk(KERN_EMERG, TAG_NAME, "drvif_TC_Set_By_DMA, table NULL, p_lut_coef_ptr=%p, addr = %d, vi_addr = %p\n",
...
char drvif_color_set_TC_Write_I_LUT_By_DMA(unsigned short *i_lut_coef_ptr, unsigned int addr, unsigned int* vir_addr)
{
    ...
    //rtd_printk(KERN_INFO, TAG_NAME, "drvif_color_set_TC_Write_I_LUT_By_DMA :  tc_i_lut : phys %x , virtaddr %p \n",addr, vir_addr );

    if ((i_lut_coef_ptr == NULL)  || (addr == 0) || (vir_addr == NULL)) {
           rtd_printk(KERN_EMERG, TAG_NAME, "drvif_TC_Set_By_DMA, table NULL, i_lut_coef_ptr=%p, addr = %d, vi_addr = %p\n",
...
char drvif_color_set_TC_Write_S_LUT_By_DMA(unsigned short *s_lut_coef_ptr, unsigned int addr, unsigned int* vir_addr)
{
    ...
    //rtd_printk(KERN_INFO, TAG_NAME, "%d, %p, {Slut dma}printk\n",addr, vir_addr );

    if ((s_lut_coef_ptr == NULL)  || (addr == 0) || (vir_addr == NULL)) {
           rtd_printk(KERN_EMERG, TAG_NAME, "drvif_TC_Set_By_DMA, table NULL, s_lut_coef_ptr=%p, addr = %d, vi_addr = %p\n",
