...
unsigned char drvif_framesync_detect(void)
{
	...
	if((Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC) == TRUE) &&
		...
		(!((Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE) > DATAFS_DISABLE_GATTING_WIDTH) && 
		(Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN_PRE) > DATAFS_DISABLE_GATTING_HEIGHT))))
		return TRUE;
#endif
...
unsigned char VODMA_get_framesync_vact_limit(unsigned int VoClk, unsigned int DClk, unsigned int VoVAct)
{
	ppoverlay_dh_total_last_line_length_RBUS ppoverlay_dh_total_last_line_length_reg;
	unsigned int VoVAct_min = 0;
	unsigned int DVAct = 0, DHTotal = 0;

	ppoverlay_dh_total_last_line_length_reg.regValue = IoReg_Read32(PPOVERLAY_DH_Total_Last_Line_Length_reg);
	DHTotal = ppoverlay_dh_total_last_line_length_reg.dh_total + 1;
	DVAct =  Scaler_DispWindowGet().src_height;
	if(DClk == 0){
		if(Get_DISPLAY_CLOCK_TYPICAL() != 0)
			DClk = (Get_DISPLAY_CLOCK_TYPICAL() / 1000);
		else
			DClk = 594000;
	}

	//ModulePrintf(LOGLEVEL_WARNING, MODULE_VOUT,"[Framesync] DVAct(%d), DClk(%d), VoClk(%d), DHTotal(%d), DVAct(%d)\n", DVAct, DClk, VoClk, DHTotal, DVAct);

	VoVAct_min = VoClk * DHTotal / DClk * DVAct / 8191;
	//ModulePrintf(LOGLEVEL_WARNING, MODULE_VOUT,"[Framesync] VoVAct_min = %d\n", VoVAct_min);

	if(VoVAct > VoVAct_min)
		return TRUE;	//Vsta is not over limit
	else
		return FALSE;	//Vsta is over limit
}
...
unsigned int VODMA_get_datafs_increased_clock(unsigned int orig_ratio_n_off)
{
	...
	if(vbe_get_VDEC4K_run_datafs_without_gatting_condition()){
		...
		//if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) {
			if(multiple > 0)
			{
				if (Scaler_DispGetInputInfo(SLR_INPUT_H_LEN) <= 4400)
				sample_rate = drvif_framesync_get_vo_clk() * remove / multiple / 10000;
			else
					sample_rate = Scaler_DispGetInputInfo(SLR_INPUT_H_LEN) * Scaler_DispGetInputInfo(SLR_INPUT_V_LEN) * 60 * remove / multiple / 10000;
			}
			else
			...
			if ((vodmapll/64*63) > (sample_rate * 10000)) {
				if((vodmapll/10000) > 0)
					...
					ratio_n_off = 64 - (sample_rate * 64 / 59400) - 1;
			}
			else
				ratio_n_off = 0;
			...
			if(Get_DISPLAY_CLOCK_TYPICAL() != 0)
				...
				DClk = 594000;
			if(multiple > 0)
				DClk = DClk * remove / multiple;
			else
				rtd_printk(KERN_EMERG, TAG_NAME_VBE, "[ERR] wrong multiple value (%d)!!!\n", multiple);
			VoClk = vodmapll / 1000;
			VoClk = VoClk * (64 - ratio_n_off) / 64;

			if(VODMA_get_framesync_vact_limit(VoClk, DClk, pVOInfo->v_length) == FALSE){
				//use den + typical porch size to calculate sample rate
				//VO Hstart=320, Vstart=26
				sample_rate = (pVOInfo->src_h_wid + (320 * 2)) * (pVOInfo->src_v_len + (26 * 2)) * 60 * remove / multiple / 10000;
			if ((vodmapll/64*63) > (sample_rate * 10000)) {
					ratio_n_off = 64 - (sample_rate * 64 / (vodmapll/10000)) - 1;
			}
			else
				ratio_n_off = 0;
				pr_info("[FRAMESYNC] VO htotal over limit, reduce sample_rate to %d, ratio_n_off(%d)\n", sample_rate, ratio_n_off);
			}
...
void drvif_framesync_gatting(unsigned char enable)
{
	...
	if(enable){
		...
		if(ori_voclk_ratio_num == 0)
		{
			...
			{
				if(vbe_get_VDEC4K_run_datafs_without_gatting_condition())
					ratio_num = VODMA_VODMA_CLKGEN_get_vodma_clk_ratio_n_off(IoReg_Read32(voAddr));
                else
		...
		printk("drvif_framesync_gatting/7484 changing voAddr=%x, clkgen = %x\n", voAddr, clkgen_reg);
