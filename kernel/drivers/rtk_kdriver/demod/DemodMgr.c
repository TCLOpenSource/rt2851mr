...
static long rtk_demod_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_RTK_DEMOD_BLINDSCAN_ACQUIRE_SIGNAL:
		...
		if (copy_from_user((void *)&pDemod_globe->blindscan_acquire_signal_data, (const void __user *)arg, sizeof(RTK_DEMOD_BLINDSCAN_ACQUIRE_SIGNAL))) {
			...
			pDemod_globe->m_pDemod->BlindScan_AcquireSignal(pDemod_globe->m_pDemod, &(pDemod_globe->blindscan_acquire_signal_data.FrequencyKhz), &(pDemod_globe->blindscan_acquire_signal_data.SymbolRate), &(pDemod_globe->blindscan_acquire_signal_data.Fcfo), &(pDemod_globe->blindscan_acquire_signal_data.bTpLock));
			ret = 0;
