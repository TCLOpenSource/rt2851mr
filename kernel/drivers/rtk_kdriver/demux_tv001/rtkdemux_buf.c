...
#include "rtkdemux_ta_4k.h"
#include "rtkdemux_ta_ecp.h"
...
extern void dump_vmallocinfo(void);
...
int DEMUX_BUF_AllocResevredSVPBuffer(DEMUX_BUF_T *pSvpBuf)
{
    ...
    totalSize = carvedout_buf_query(CARVEDOUT_DTV_VDEC_RINGBUF, (void**)&startPhyAddr);
    dmx_info("carved-out svp buffer startPhyAddr:0x%x totalSize:%d\n",startPhyAddr,totalSize);
    if (totalSize > 0) {
        dmx_err("Using Reserved Buffer phyAddr: 0x%x\n",startPhyAddr);
        pSvpBuf->phyAddr = startPhyAddr;
        pSvpBuf->allocSize = totalSize ;
        pSvpBuf->cachedaddr = dvr_remap_cached_memory(pSvpBuf->phyAddr, totalSize, __builtin_return_address(0));
        pSvpBuf->nonCachedaddr = dvr_remap_uncached_memory(pSvpBuf->phyAddr, totalSize, __builtin_return_address(0));
        pSvpBuf->isCache = 0;
        pSvpBuf->virAddr = pSvpBuf->isCache ? pSvpBuf->cachedaddr : pSvpBuf->nonCachedaddr ;
        pSvpBuf->size = totalSize ;
        pSvpBuf->fromPoll = 0 ;
        pSvpBuf->fromVBM = 0;

        dmx_info("[%s]cachedAddr 0x%lx, uncachedAddr 0x%lx, phyAddr 0x%lx, allocSize 0x%x\n",
            __func__, (unsigned long)(uintptr_t)pSvpBuf->cachedaddr,
            (unsigned long)(uintptr_t)pSvpBuf->nonCachedaddr,	(unsigned long)pSvpBuf->phyAddr, pSvpBuf->allocSize);
        } else {
        dmx_warn("%s_%d: carvedout SVP buffer allocate FAIL \n!!!!!", __func__, __LINE__);
        return -1;
    }
    demux_device->usedSVPPollSize = 0;
    return 0;
}

void DEMUX_BUF_ReleaseResevredSVPBuffer(DEMUX_BUF_T *pSvpBuf)
{
    unsigned int svp_buf_start = 0;
    unsigned int svp_buf_size  = 0;
    svp_buf_size = carvedout_buf_query(CARVEDOUT_DTV_VDEC_RINGBUF, (void**)&svp_buf_start);
    if (pSvpBuf->phyAddr >= svp_buf_start && (pSvpBuf->phyAddr + pSvpBuf->allocSize) <= (svp_buf_start + svp_buf_size)) {
        dvr_unmap_memory((void *)pSvpBuf->cachedaddr, pSvpBuf->allocSize);
        dvr_unmap_memory((void *)pSvpBuf->nonCachedaddr, pSvpBuf->allocSize);
        dmx_info("[%s %d]return carved-out svp buffer.start addr:0x%x, size:%d\n", __func__, __LINE__, svp_buf_start, svp_buf_size);
        memset(pSvpBuf, 0, sizeof(DEMUX_BUF_T));
    }
    demux_device->usedSVPPollSize = 0;
}
...
{
	pBuf->allocSize = (size + 0xf) & ~0xf ; /*size < 0xf ? 0xf : size ;*/

	if (pBuf->allocSize + demux_device->usedSVPPollSize > pSrcBuf->allocSize) {
		dmx_err("RTKDEMUX: cannot allocate memory from pool, pool size = %x, used size = %x, alloc size = %x\n", pSrcBuf->allocSize, demux_device->usedSVPPollSize, size) ;
		return -EFAULT ;
	}
	pBuf->phyAddr       = (dma_addr_t)((unsigned int)pSrcBuf->phyAddr + demux_device->usedSVPPollSize) ;
	pBuf->cachedaddr    = pSrcBuf->cachedaddr + demux_device->usedSVPPollSize ;;
	pBuf->nonCachedaddr = pSrcBuf->nonCachedaddr + demux_device->usedSVPPollSize ;
	pBuf->virAddr       = isCache ? pBuf->cachedaddr : pBuf->nonCachedaddr ;
	pBuf->isCache       = isCache ;
	pBuf->fromPoll      = 1 ;
	pBuf->size          = size ;

	demux_device->usedSVPPollSize += pBuf->allocSize ;
	dmx_err("RTKDEMUX: Poll: cachedAddr %lx, uncachedAddr %lx, phyAddr %lx, allocSize %x\n",
...
int DEMUX_BUF_ReturnDecBuffer(demux_channel *pCh, int pin)
{
    ...
    if(pBufBS->phyAddr && IS_VIDEO_PIN(pin)){
        DEMUX_BUF_ZeroOutBuffer(pBufBS);
        dmx_crit("RTKDEMUX: ZeroOutBuffer es svp buffer success!!!");
    }
    else if (pBufBS->phyAddr) {
        if( RHAL_Delivery_GetMode(pCh->tunerHandle) == TP_DELIVERY_MODE_TS ){
            dmx_crit("[%s %d]CH%d return pin %d BS buf = 0x%lx\n", __func__, __LINE__, pCh->tunerHandle, pin, (unsigned long)pBufBS->phyAddr);
            if (pCh->bUseSelfBuf)
...
void DEMUX_BUF_UpdatePinInfo(demux_channel *pCh)
{
	...
	for (i = VIDEO_PIN; i < NUMBER_OF_PINS; i++) {
		...
		if (pCh->pinInfo[i].bufStatus == BUF_STAT_FREE) {
			...
			if(RHAL_Delivery_GetMode(pCh->tunerHandle) == TP_DELIVERY_MODE_JAPAN4K){
				dmx_emerg("[%s %d] BUF_STAT_FREE \n", __func__, __LINE__);
#if 0
				if(IS_AUDIO_PIN(i)){
					RHAL_TP_DisableMemProtect(pCh->tunerHandle, MEMORY_PRISON_AUDIO_ES);
				}
				if(IS_VIDEO_PIN(i)){
					RHAL_TP_DisableMemProtect(pCh->tunerHandle, MEMORY_PRISON_VIDEO_ES);
				}
			...
			if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS){
				dmx_emerg("[%s %d] BUF_STAT_NEW \n", __func__, __LINE__);
				DEMUX_TA_4K_UpdatePininfo(DEMUX_STREAM_TYPE_IB, i, pBufIBH, pBufIB);
				DEMUX_TA_4K_UpdatePininfo(DEMUX_STREAM_TYPE_BS, i, pBufBSH, pBufBS);
#if 0
				if(IS_AUDIO_PIN(i)){
					RHAL_TP_EnableMemProtect(pCh->tunerHandle, MEMORY_PRISON_AUDIO_ES, pBufBS->phyAddr,  pBufBS->phyAddr+pBufBS->size);
				}
				if(IS_VIDEO_PIN(i)){
			...
			dmx_emerg("[%s %d]BS rp=0x%lx, wp = 0x%lx, base = 0x%lx, head phy:0x%lx, bs:0x%lx\n", __func__, __LINE__,
				(unsigned long)pHeader->readPtr[0], (unsigned long)pHeader->writePtr, (unsigned long)pHeader->beginAddr,
				(unsigned long)pBufBSH->phyAddr, (unsigned long)pBufBS->phyAddr);
...
int DEMUX_BUF_PrepareSelfDecBuf(demux_channel *pCh, int pin, int bsSize, int ibSize)
{
    ...
    if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS){
        if (pBufBS->virAddr == 0 ){
            if(DEMUX_BUF_AllocFromSVPPoll(pBufBS, bsSize, &demux_device->svpBufPoll, 0) < 0){
                dmx_crit("[%s %d]CH%d out of memory: allocate bs buffer fail. bsSize = %d\n", __func__, __LINE__, pCh->tunerHandle, bsSize);
                return -1;
            }
        }
        if (DEMUX_BUF_AllocFromSVPPoll(pBufBSH, sizeof(RINGBUFFER_HEADER), &demux_device->svpBufPoll, 0) < 0) {
        ...
        if (ibSize > 0) {
            if (DEMUX_BUF_AllocFromSVPPoll(pBufIBH, sizeof(RINGBUFFER_HEADER), &demux_device->svpBufPoll, 0) < 0) {
                dmx_crit("[%s %d]CH%d out of memory: allocate buffer header fail\n", __func__, __LINE__, pCh->tunerHandle);
                return -1;
            }
            if (pBufIB->virAddr == 0 && DEMUX_BUF_AllocFromSVPPoll(pBufIB, ibSize, &demux_device->svpBufPoll, 0) < 0) {
...
#endif
        if (pBufBSH->virAddr == 0 && DEMUX_BUF_AllocBuffer(pBufBSH, sizeof(RINGBUFFER_HEADER), 0, 1) < 0) {
            dmx_crit("[%s %d]CH%d out of memory: allocate buffer header fail\n", __func__, __LINE__, pCh->tunerHandle);
            return -1;
        }

        if (pBufBS->virAddr == 0 && IS_VIDEO_PIN(pin)){
            if(DEMUX_BUF_AllocFromSVPPoll(pBufBS, bsSize, &demux_device->svpBufPoll, 1) < 0){
                dmx_crit("[%s %d]CH%d out of memory: allocate bs buffer fail. bsSize = %d\n", __func__, __LINE__, pCh->tunerHandle, bsSize);
                return -1;
            }
        }
...
#endif
            if (pBufIBH->virAddr == 0 && DEMUX_BUF_AllocBuffer(pBufIBH, sizeof(RINGBUFFER_HEADER), 0, 1) < 0) {
                dmx_crit("[%s %d]CH%d out of memory: allocate buffer header fail\n", __func__, __LINE__, pCh->tunerHandle);
                return -1;
            }
...
#ifdef USE_STATIC_RESERVED_BUF
	{
		...
		if (totalSize > 0) {
			...
			if ((phyAddr+DEMUX_TP_BUFF_SIZE) <= (startPhyAddr + totalSize)) {
				...
				if(pBuf->cachedaddr == NULL || pBuf->nonCachedaddr == NULL){
					dmx_err("++ dump vmallocinfo \n");
					dump_vmallocinfo();
					dmx_err("-- dump vmallocinfo \n");
