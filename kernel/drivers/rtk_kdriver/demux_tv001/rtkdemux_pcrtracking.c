...
#define PCRTRACK_PREDEFINED_FULL_SPAN                  120 //320 // 256
					...
					if (pcr_diff > 0 && pcr_diff1 > 0  && stc_diff > 0 && stc_diff1 > 0 && abs(rcd - rcd1) < DEMUX_CLOCKS_PER_SEC && abs(rcd1 - rcd2) < DEMUX_CLOCKS_PER_SEC)
					{
						...
						else
						{
							...
							else
							{
								...
								{
									//speedAdjust++;
									speedAdjust = speedAdjust + (int)preAdjust/10 + 1;
								}
								else if (rcd_diff < 0)
								{
									//speedAdjust--;
...
static int PCRTRACK_GetNextSpeedPredefinedArrayIdx(PCRTRK_MANAGER_T *pManager, int step)
{
	...
	if(step<-PCRTRACK_MAX_STEP)
		step = -PCRTRACK_MAX_STEP;
	else if(step > PCRTRACK_MAX_STEP)
		step = PCRTRACK_MAX_STEP;
