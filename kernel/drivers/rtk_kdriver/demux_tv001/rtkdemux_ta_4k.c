...
#define JP4K_TA_UUID { 0xa409ae4e, 0x31e0, 0x4163, \
    { 0xba, 0x87, 0x01, 0xec, 0x94, 0x2d, 0x54, 0xd6} }

#define ATSC30_ALP_TA_UUID { 0xbe22e836, 0x9bf0, 0x11ea, \
...
{
    int ret = 0, rc = 0;
    //struct teec_uuid uuid = JP4K_TA_UUID;
    struct tee_param param[TEE_NUM_PARAM];
    ...
    if(RHAL_Delivery_GetMode(0) == TP_DELIVERY_MODE_JAPAN4K){
        struct teec_uuid uuid = JP4K_TA_UUID;
        memcpy(&arg.uuid, &uuid, sizeof(struct teec_uuid));
    }else{
        struct teec_uuid uuid = ATSC30_ALP_TA_UUID;
        memcpy(&arg.uuid, &uuid, sizeof(struct teec_uuid));
    }
    //memcpy(&arg.uuid, &uuid, sizeof(struct teec_uuid));
