...
#ifdef CONFIG_RTK_KDRV_JP4K
extern UINT64 japan4k_dmx_get_RCD(unsigned char tp_id);
#endif
...
void Demux_Enter_ECP(demux_channel *pCh)
{
	...
	set_dtv_securestatus_aipq(TRUE);// 0520 lsy
	#endif
...
void Demux_Exit_ECP(demux_channel *pCh)
{
	...
	#if defined(CONFIG_RTK_AI_DRV)
	set_dtv_securestatus_aipq(FALSE);// 0520 lsy
	#endif
	pCh->isECP = 0;
...
int DeliverPrivateInfo(demux_channel *pCh, DEMUX_BUF_PIN_T *pPinInfo, int infoId, char *pInfo, int length, DEMUX_BUF_PIN_T *pBSPinInfo, int pin)
{
	int res = -2;
	...
	if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS){
		res = DEMUX_TA_4K_DeliverPrivateInfo(pin, infoId, pInfo, length);
		return res;
	}
...
#define NAV_SYNC_WRITE_POINTERS() \
	...
	for (i = 0 ; i < pDemuxOut->numBuffers ; i++) {
		...
		if( !IS_AUDIO_PIN(pNavBuffer->data.pinIndex) ) {
		...
		if (pNavBuffer->data.infoId != PRIVATEINFO_NONE) {
			NAV_SYNC_WRITE_POINTERS();
			if (DeliverPrivateInfo(pCh, pPinIBInfo, pNavBuffer->data.infoId, pNavBuffer->data.infoData, pNavBuffer->data.infoSize, pPinInfo, pNavBuffer->data.pinIndex) < 0) {
		...
		if (pNavBuffer->data.payloadSize > 0) {
			...
			if (pNavBuffer->data.pts >= 0) {
				...
				if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_PTS, (char *)&pNavBuffer->data.pts, sizeof(int64_t), pPinInfo, pNavBuffer->data.pinIndex) < 0) {
					/*                    assert(demuxIn.reservedInfoId == PRIVATEINFO_NONE);*/
					pCh->demuxIn.reservedInfoId = pNavBuffer->data.infoId;
					pCh->demuxIn.reservedInfoSize = pNavBuffer->data.infoSize;
					pCh->demuxIn.reservedInfoData = pNavBuffer->data.infoData;
...
void DEMUX_HandleCriticalEvents(demux_channel *pCh)
{
	...
	if (HAS_FLAG(pCh->status, STATUS_FLUSH_SUBTITLE)) {
		...
		if (pPinIBInfo->pRBH) {
			...
			DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_VIDEO_NEW_SEG, NULL, sizeof(NEW_SEG), pPinInfo, SUBTITLE_PIN);
...
void HandleEvents(demux_channel *pCh, NAVBUF *pBuffer)
{
	if (HAS_FLAG(pCh->events, EVENT_VIDEO0_NEW_DECODE_MODE)) {
		DeliverPrivateInfo(pCh, &pCh->pinInfo[VIDEO_PIN].bufInfo[DEMUX_STREAM_TYPE_IB], PRIVATEINFO_VIDEO_NEW_DECODE_MODE, (char *)&pCh->videoDecodeMode[0], sizeof(pCh->videoDecodeMode[0]), &pCh->pinInfo[VIDEO_PIN].bufInfo[DEMUX_STREAM_TYPE_BS], VIDEO_PIN);
		RESET_FLAG(pCh->events, EVENT_VIDEO0_NEW_DECODE_MODE);
	}
	if (HAS_FLAG(pCh->events, EVENT_VIDEO1_NEW_DECODE_MODE)) {
		DeliverPrivateInfo(pCh, &pCh->pinInfo[VIDEO2_PIN].bufInfo[DEMUX_STREAM_TYPE_IB], PRIVATEINFO_VIDEO_NEW_DECODE_MODE, (char *)&pCh->videoDecodeMode[1], sizeof(pCh->videoDecodeMode[1]), &pCh->pinInfo[VIDEO2_PIN].bufInfo[DEMUX_STREAM_TYPE_BS], VIDEO2_PIN);
...
int DEMUX_EVENT_StartVideo(demux_channel *pCh, DECODE_MODE decMode, int pin)
{
	...
	pPinInfo   = &pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS];
	pPinIBInfo = &pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB];
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_VIDEO_NEW_SEG, NULL, sizeof(NEW_SEG), pPinInfo, pin) < 0) {
		dmx_mask_print(DEMUX_NOMAL_DEBUG, "%s, %d: ch:%d Deliver PRIVATEINFO_VIDEO_NEW_SEG FAIL\n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}
	/* ANDROIDTV-227 issue */
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_VIDEO_INBAND_CMD_TYPE_SOURCE_DTV, NULL, sizeof(INBAND_SOURCE_DTV), pPinInfo, pin) < 0) {
		dmx_mask_print(DEMUX_NOMAL_DEBUG, "%s, %d: ch:%d Deliver PRIVATEINFO_VIDEO_INBAND_CMD_TYPE_SOURCE_DTV FAIL\n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_VIDEO_DECODE_COMMAND, (char *)&cmd, sizeof(DECODE), pPinInfo, pin) < 0) {
		dmx_mask_print(DEMUX_NOMAL_DEBUG, "%s, %d: ch:%d Deliver PRIVATEINFO_VIDEO_DECODE_COMMAND FAIL\n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}

	if (pCh->tp_src != MTP){
		unsigned char delivery_mode;
		delivery_mode = RHAL_Delivery_GetMode(pCh->tunerHandle);
		if(delivery_mode == TP_DELIVERY_MODE_ATSC30 || delivery_mode == TP_DELIVERY_MODE_ATSC30_ALP)
...
int DEMUX_EVENT_StartAudio(demux_channel *pCh, int pin)
{
	...
	pPinInfo   = &pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS];
	pPinIBInfo = &pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB];
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_AUDIO_NEW_SEG, NULL, sizeof(AUDIO_DEC_NEW_SEG), pPinInfo, pin) < 0) {
		dmx_mask_print(DEMUX_NOMAL_DEBUG, "%s, %d: ch:%d Deliver PRIVATEINFO_AUDIO_NEW_SEG FAIL\n", __func__, __LINE__, pCh->tunerHandle);
		return -1;
	}
	if (DeliverPrivateInfo(pCh, pPinIBInfo, PRIVATEINFO_AUDIO_DECODE_COMMAND, (char *)&cmd, sizeof(AUDIO_DEC_DECODE), pPinInfo, pin) < 0) {
...
int DEMUX_EVENT_AudioNewFormat(demux_channel *pCh, int pin, DEMUX_AUDIO_DEC_T audioFormat, s32 privateInfo[8])
{
	...
	BufferFullDroppingReset(pCh, pin, 0);
	if (DeliverPrivateInfo(pCh, &pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_IB], PRIVATEINFO_AUDIO_FORMAT, (char *)&cmd, sizeof(AUDIO_DEC_NEW_FORMAT), &pCh->pinInfo[pin].bufInfo[DEMUX_STREAM_TYPE_BS], pin)<0)
	{
...
extern UINT32 g_jp4k_ntpoffset;
			REFCLOCK *refClock = DEMUX_GetReferenceClock(pCh);
			long long mpts = DEMUX_reverseLongInteger(refClock->RCD)+CLOCK_GetAVSyncPTS((REFCLOCK *)refClock);
			long long apts = DEMUX_reverseLongInteger(refClock->audioSystemPTS);
			long long ma_diff = mpts - apts; 
			if(pCh->avSyncMode != NAV_AVSYNC_FREE_RUN){
				if(abs(REFCLK_GetRCD(pCh->pcrProcesser.pRefClock) - japan4k_dmx_get_RCD(pCh->tunerHandle)) > 50*90){ /* RCD unstable)*/
					REFCLK_SetRCD(pCh->pcrProcesser.pRefClock, japan4k_dmx_get_RCD(pCh->tunerHandle));
				}
				if(ma_diff >= 0 && ma_diff <18000)
					g_jp4k_ntpoffset = 27*9000;
				else if (ma_diff >= 18000 && ma_diff <36000)
					g_jp4k_ntpoffset = 30*9000;
				else if(ma_diff >36000)
					g_jp4k_ntpoffset = 50*9000;
				else
					g_jp4k_ntpoffset = 25*9000;
			}
#endif
		}else if (!pCh->bIsSDT && pCh->startStreaming && pCh->avSyncMode == NAV_AVSYNC_PCR_MASTER ){
		...
		if (pDemuxIn->pCurr >= pDemuxIn->pEnd) {
			NAVBUF buf ;
			/*for TCL japan 4k, no filter and pid set, but need to sent new seg to av fw*/
			if(RHAL_Delivery_GetMode(pCh->tunerHandle) == TP_DELIVERY_MODE_TS)
...
int DEMUX_ThreadProc(demux_channel *pCh)
{
	...
	while (!kthread_should_stop()) {
		...
		if ((pCh->startStreaming || RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS) && !pCh->bIsSDT && DEMUX_HasReferenceClock(pCh)) {
