...
enum {
	...
	DEMUX_DBG_CMD_INIT_UNINIT_CHANNEL = 23,
...
static const char *rtkdemux_cmd_str[] = {
	...
	"inituninit_channel=",	/* DEMUX_DBG_CMD_ESBUF_MONITOR */
};
...
static inline void rtkdemux_dbg_EXECUTE(const int cmd_index, char **cmd_pointer)
{
	...
	case DEMUX_DBG_CMD_INIT_UNINIT_CHANNEL:
	{
		demux_channel *pCh = NULL;
		int channel = 0;
		int init = 0;
		...
		token = strsep(cmd_pointer, delim);
		if (token == NULL || kstrtoint(token, 0, &channel) != 0) {
			DMX_DBG_LOG(DGB_COLOR_RED"rtkdemux_debug:%d failed to parse the inband command enable flag.\n"DGB_COLOR_NONE, __LINE__);
			break;
		}

		token = strsep(cmd_pointer, delim);
		if (token == NULL || kstrtoint(token, 0, &init) != 0) {
			DMX_DBG_LOG(DGB_COLOR_RED"rtkdemux_debug:%d failed to parse tp index.\n"DGB_COLOR_NONE, __LINE__);
			break;
		}

		dmx_notice("DEMUX_CHANNEL_T=%d init=%d\n",  (DEMUX_CHANNEL_T)channel, init);
		pCh = &demux_device->pChArray[(DEMUX_CHANNEL_T)parsed_data];
		if(!init){
				IOCTLEX_UninitChannelImpl(pCh);
		}
		else{
				ret = IOCTLEX_InitChannelImpl(channel);
				if(ret != 0){
						dmx_err("[%s %d] IOCTLEX_InitChannelImpl failled!!!", __func__, __LINE__);
				}
		}
