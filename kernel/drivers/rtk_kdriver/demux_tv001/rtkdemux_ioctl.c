...
int IOCTL_InitModule ()
{
	...
	if(DEMUX_BUF_AllocResevredSVPBuffer(&demux_device->svpBufPoll) < 0)
	{
		dmx_err("%s_%d: FAIL !!!!!\n", __func__, __LINE__);
		goto demux_init_fail;
	}
...
int IOCTL_UnInitModule(void)
{
	...
	DEMUX_BUF_ReleaseResevredSVPBuffer(&demux_device->svpBufPoll);
...
int IOCTL_GetBufferInfo(DEMUX_BUF_INFO_T *pInfo)
{
	...
	case AUDIO2_PIN:
	{
		if (pCh->bUseSelfBuf){
			if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS){
				DEMUX_BUF_PrepareSelfDecBuf(pCh, pin, DEMUX_AD_TLV_BS_SIZE, DEMUX_AD_TLV_IB_SIZE);
			}else{
	...
	case VIDEO2_PIN:
	{
		if (pCh->bUseSelfBuf){
			if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS){
				DEMUX_BUF_PrepareSelfDecBuf(pCh, pin, DEMUX_VD_TLV_BS_SIZE, DEMUX_VD_TLV_IB_SIZE);
			}else{
...
int IOCTL_SetTPOutStreamControl(DEMUX_SET_TP_OUT_STREAM_CONTROL_T *pInfo)
{
	...
	TPK_TP_SOURCE_T tpp_src = Internal_Demod;
	TPK_TPOUT_SOURCE_T tpout_src = TPK_TPP_TO_TPOUT;
	TPK_TPOUT_STATUS_T tpout_tatus;
	UINT8 sync_byte[MAX_TP_P_COUNT] = {0x47, 0x48, 0x49};

	dmx_notice("[%s %d] Enter, pInfo->ctrl=%d\n", __func__, __LINE__, pInfo->ctrl);

	if(pInfo->ctrl == TPOUT_STREAM_START){
		...
		//enable tpo
		RHAL_TPOUT_GetStatus(0,&tpout_tatus);
		if(tpout_tatus.tpout_stream_status != TPOUT_STREAM_START){
