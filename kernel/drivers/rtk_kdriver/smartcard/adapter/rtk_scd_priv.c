...
int scd_reset(scd *p_this)
{
	...
	wait_event_interruptible_timeout(p_this->wq, p_this->atr.length > 0, HZ);	/* wait 1s */
	...
	wait_event_interruptible_timeout(p_this->wq, p_this->fsm != IFD_FSM_RESET, HZ);	/*  wait 1s  */
...
static void scd_load_gpio_config(scd *p_this)
{
	...
	rtd_outl(0xB80008F8, (rtd_inl(0xB80008F8) & ~(0x0 << 4))); /* IO data in input mux, select src0 */
	...
	rtd_outl(0xB8000844, rtd_inl(0xB8000844) | (0xF << 20));    /*PWR SEL  : MISC GPIO 62 AA9*/
	rtd_outl(0xB8000894, rtd_inl(0xB8000894) | (0xF << 4));    /* CMD VCC : MISC GPIO 126 AA26*/
