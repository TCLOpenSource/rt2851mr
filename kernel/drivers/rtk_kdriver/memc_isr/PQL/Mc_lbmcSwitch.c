...
static VOID LbmcMode_Detect_onlyCnt(const _PARAM_MC_LBMC_SWITCH *pParam, _OUTPUT_MC_LBMC_SWITCH *pOutput, UBYTE band_Type)
{
	...
	// mode switch
	{
		...
		if(0)
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "ovp_posCnt = %d, ovp_negCnt = %d, Lmv_posCnt = %d, Lmv_negCnt = %d, total_cnt = %d \r\n", \
	...
	if(0)
	{
		if(band_Type == _LBMC_LF_)
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "lf_lbmc_mode = %d \r\n", pOutput->u8_lf_lbmcMode);
		}
		else
		{
			rtd_printk(KERN_NOTICE, TAG_NAME_MEMC, "Hf_lbmc_mode = %d \r\n", pOutput->u8_hf_lbmcMode);
		}
	}
