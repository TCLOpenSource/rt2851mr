...
#define	REDUCE_SDNR_WIDTH_LIMIT_THRESHOLD		4094//this is for RTRN input max size
//[crixus]scart out
...
typedef enum _MCAP_VI_DMA_MODE_T_{
    VI_DMA_MODE_SEQUENCE = 0,
    VI_DMA_MODE_BLOCK = 1
}MCAP_VI_DMA_MODE_ET;
...
typedef struct {	//initialization in scalerstruct.c
	...
	unsigned char Gamemode_break_flag;
	unsigned char game_mode_rerun_scaler_flag;
...
typedef enum
{
  ...
  SCALERIOC_AV_SYNC_IDX,//for vdec mode to check av sync
	...
	CINEMA_MODE_ENABLE_IDX,
	VO_TRACK_ENABLE_IDX,
	VO_FORCE_60_FRAMERATE_IDX,
...
typedef struct _SCALER_DATA_FS_ALGO_RESULT {
	unsigned int ulEnableVCrop;
	unsigned int ulUpdateReg;
