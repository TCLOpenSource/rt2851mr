...
#ifdef CONFIG_ARM64
struct RPC_STRUCT {
	u_int programID;	   // program ID defined in IDL file
	u_int versionID;	   // version ID defined in IDL file
	u_int procedureID;    // function ID defined in IDL file

	u_int taskID;		   // the caller's task ID, assign 0 if NONBLOCK_MODE
#ifdef RTD_1195
	u_int sysPID;		   // the callee's task ID
#endif
	u_int parameterSize;  // packet's body size

#ifdef CONFIG_RTK_KDRIVER_SUPPORT	 
	u_int mycontext;		  // return address of reply value
#else
...
#ifdef CONFIG_RTK_KDRIVER_SUPPORT    
	u_long mycontext;         // return address of reply value
#else
	u_int context;         // return address of reply value
#endif
};
