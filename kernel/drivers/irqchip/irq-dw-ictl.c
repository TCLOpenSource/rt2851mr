...
static inline void ictl_w32(u32 val, unsigned reg)
{
	__raw_writel(val, ictl_membase + ictl_regs[reg]);
}

static inline u32 ictl_r32(unsigned reg)
{
...
{
	ictl_w32(BIT(d->hwirq), ICTL_REG_MASK);
}
...
{
	unsigned int pending = ictl_r32(ICTL_REG_FINALSTATUS);
				  ...
				  unsigned int cpu_vec)
{
	...
	ictl_w32(0xffffffff, ICTL_REG_MASK);
	ictl_w32(0xffffffff, ICTL_REG_INTEN);
