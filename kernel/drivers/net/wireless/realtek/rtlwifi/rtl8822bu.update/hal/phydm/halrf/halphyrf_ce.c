...
#if CONFIG_TEMP_RECOVERY_TV002
	if (dm->support_ic_type & ODM_RTL8822B) {
		if (thermal_value >= 0x37 && rf->high_ther == 0) {
			RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			       "AVG Thermal Meter = 0x%X >= 0x37, Set 1 Tx\n",
			       thermal_value);
			config_phydm_trx_mode_8822b(dm, 1, 3, 0);
			rf->high_ther = 1;
		}
		if (thermal_value <= 0x30 && rf->high_ther == 1) {
			RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			       "AVG Thermal Meter = 0x%X <= 0x30, Set 2 Tx\n",
			       thermal_value);
