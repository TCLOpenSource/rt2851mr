...
u32 glcoex_ver_date_8822b_1ant = 20180816;
u32 glcoex_ver_8822b_1ant = 0x65;
					    ...
					    u8 *tmp_buf, u8 length)
{
	...
	static boolean is_no_wl_5ms_extend;
	...
	if (coex_sta->force_lps_ctrl) {
		...
		if (cnt == 7) {
			...
			is_no_wl_5ms_extend = TRUE;
		}
	}

	if (is_no_wl_5ms_extend && coex_sta->cck_lock) {
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
