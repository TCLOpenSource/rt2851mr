...
#define KEEP_ALIVE_TRYCNT (3)
	...
	if (chk_alive_num) {
		...
		for (i = 0; i < chk_alive_num; i++) {
			...
			#ifdef CONFIG_RTW_MESH
			if (MLME_IS_MESH(padapter)) {
				#ifdef CONFIG_RTW_MESH_DRIVER_STA_DISASOC
				if (rtw_mesh_plink_del(padapter, sta_addr) == _SUCCESS)
					rtw_cfg80211_indicate_sta_disassoc(padapter, sta_addr, 0);
				#else
				rtw_mesh_plink_set_state(padapter, sta_addr, RTW_MESH_PLINK_UNKNOWN);
				#endif
			}
			#endif
		}
