...
#define RTW_TOTAL_PKT_MIN_THRESHOLD 10
	...
	*pos++ = ttl;
	if (mpath_action == RTW_MPATH_PREP) {
		...
		*(u32 *)pos = cpu_to_le32(target_sn);
		...
		_rtw_memcpy(pos, orig_addr, ETH_ALEN);
		pos += ETH_ALEN;
		*(u32 *)pos = cpu_to_le32(orig_sn);
		pos += 4;
	}
			   ...
			   u16 target_rcode, const u8 *ra)
{
	...
	*(u32 *)pos = cpu_to_le32(target_sn);
	pos += 4;
	*(u16 *)pos = cpu_to_le16(target_rcode);

	/* deferred tx is needed? */
	/* rtw_prepare_frame_for_deferred_tx(); */
				  ...
				  u8 bw, u8 total_pkt)
{
	...
	/* Need update policy */
#if 0
	if (rpt->total_pkt < RTW_TOTAL_PKT_MIN_THRESHOLD)
		return;
#endif
				   ...
				   const u8 *hwmp_ie, enum rtw_mpath_frame_type action)
{
	...
	switch (action) {
	case RTW_MPATH_PREQ:
		orig_addr = RTW_PREQ_IE_ORIG_ADDR(hwmp_ie);
		orig_sn = RTW_PREQ_IE_ORIG_SN(hwmp_ie);
		orig_lifetime = RTW_PREQ_IE_LIFETIME(hwmp_ie);
		orig_metric = RTW_PREQ_IE_METRIC(hwmp_ie);
		break;
	case RTW_MPATH_PREP:
		 ...
		 */
		orig_addr = RTW_PREP_IE_TARGET_ADDR(hwmp_ie);
		orig_sn = RTW_PREP_IE_TARGET_SN(hwmp_ie);
		orig_lifetime = RTW_PREP_IE_LIFETIME(hwmp_ie);
		orig_metric = RTW_PREP_IE_METRIC(hwmp_ie);
		break;
	default:
		rtw_rcu_read_unlock();
		return 0;
	...
	exp_time = RTW_TU_TO_EXP_TIME(orig_lifetime);

	if (rtw_ether_addr_equal(orig_addr, adapter_mac_addr(adapter))) {
		...
		mpath = rtw_mesh_path_lookup(adapter, orig_addr);
		if (mpath) {
			enter_critical_bh(&mpath->state_lock);
			if (mpath->flags & RTW_MESH_PATH_FIXED)
				fresh_info = _FALSE;
			else if ((mpath->flags & RTW_MESH_PATH_ACTIVE) &&
			    (mpath->flags & RTW_MESH_PATH_SN_VALID)) {
				if (RTW_SN_GT(mpath->sn, orig_sn) ||
				    (mpath->sn == orig_sn &&
				...
				have_sn = mpath->flags & RTW_MESH_PATH_SN_VALID;
				newer_sn = have_sn && RTW_SN_GT(orig_sn, mpath->sn);
				bounced = have_sn &&
					  (RTW_SN_DELTA(orig_sn, mpath->sn) >
			...
			mpath = rtw_mesh_path_add(adapter, orig_addr);
			if (IS_ERR(mpath)) {
				rtw_rcu_read_unlock();
				return 0;
			}
		...
		if (fresh_info) {
			rtw_mesh_path_assign_nexthop(mpath, sta);
			mpath->flags |= RTW_MESH_PATH_SN_VALID;
			mpath->metric = new_metric;
			mpath->sn = orig_sn;
			mpath->exp_time = rtw_time_after(mpath->exp_time, exp_time)
	...
	ta = mgmt->addr2;
	if (rtw_ether_addr_equal(orig_addr, ta))
		fresh_info = _FALSE;
	else {
		fresh_info = _TRUE;

		mpath = rtw_mesh_path_lookup(adapter, ta);
		if (mpath) {
			enter_critical_bh(&mpath->state_lock);
			if ((mpath->flags & RTW_MESH_PATH_FIXED) ||
			...
			mpath = rtw_mesh_path_add(adapter, ta);
			if (IS_ERR(mpath)) {
				rtw_rcu_read_unlock();
				return 0;
			}
					...
					const u8 *preq_elem, u32 orig_metric)
{
	...
	orig_addr = RTW_PREQ_IE_ORIG_ADDR(preq_elem);
	target_sn = RTW_PREQ_IE_TARGET_SN(preq_elem);
	orig_sn = RTW_PREQ_IE_ORIG_SN(preq_elem);
	...
	if (reply) {
		...
		if (ttl != 0) {
			...
			rtw_mesh_path_sel_frame_tx(RTW_MPATH_PREP, 0, orig_addr,
	...
	if (forward && mshcfg->dot11MeshForwarding) {
		...
		rtw_mesh_path_sel_frame_tx(RTW_MPATH_PREQ, flags, orig_addr,
					   orig_sn, target_flags, target_addr,
					   target_sn, da, hopcount, ttl, lifetime,
					   orig_metric, preq_id, adapter);
...
rtw_next_hop_deref_protected(struct rtw_mesh_path *mpath)
{
	return rtw_rcu_dereference_protected(mpath->next_hop,
					 rtw_lockdep_is_held(&mpath->state_lock));
}
					...
					const u8 *prep_elem, u32 metric)
{
	...
	orig_addr = RTW_PREP_IE_ORIG_ADDR(prep_elem);
	if (rtw_ether_addr_equal(orig_addr, adapter_mac_addr(adapter)))
	...
	_rtw_memcpy(next_hop, rtw_next_hop_deref_protected(mpath)->cmn.mac_addr, ETH_ALEN);
	exit_critical_bh(&mpath->state_lock);
	--ttl;
	...
	orig_sn = RTW_PREP_IE_ORIG_SN(prep_elem);

	rtw_mesh_path_sel_frame_tx(RTW_MPATH_PREP, flags, orig_addr, orig_sn, 0,
					...
					const u8 *perr_elem)
{
	...
	target_rcode = RTW_PERR_IE_TARGET_RCODE(perr_elem);
	...
	if (mpath) {
		...
		enter_critical_bh(&mpath->state_lock);
		sta = rtw_next_hop_deref_protected(mpath);
		if (mpath->flags & RTW_MESH_PATH_ACTIVE &&
		    rtw_ether_addr_equal(ta, sta->cmn.mac_addr) &&
		    !(mpath->flags & RTW_MESH_PATH_FIXED) &&
		    (!(mpath->flags & RTW_MESH_PATH_SN_VALID) ||
		    RTW_SN_GT(target_sn, mpath->sn)  || target_sn == 0)) {
			mpath->flags &= ~RTW_MESH_PATH_ACTIVE;
			if (target_sn != 0)
				mpath->sn = target_sn;
			else
					...
					const struct rtw_ieee80211_rann_ie *rann)
{
	...
	RTW_HWMP_DBG("received RANN from "MAC_FMT" via neighbour "MAC_FMT" (is_gate=%d)\n",
		  MAC_ARG(orig_addr), MAC_ARG(mgmt->addr2), root_is_gate);
	...
	if (!(RTW_SN_LT(mpath->sn, orig_sn)) &&
	    !(mpath->sn == orig_sn && metric < mpath->rann_metric)) {
		rtw_rcu_read_unlock();
		return;
	}

	if ((!(mpath->flags & (RTW_MESH_PATH_ACTIVE | RTW_MESH_PATH_RESOLVING)) ||
	     (rtw_time_after(rtw_get_current_time(), mpath->last_preq_to_root +
				  rtw_root_path_confirmation_jiffies(adapter)) ||
	     rtw_time_before(rtw_get_current_time(), mpath->last_preq_to_root))) &&
	     !(mpath->flags & RTW_MESH_PATH_FIXED) && (ttl != 0)) {
		RTW_HWMP_DBG("time to refresh root mpath "MAC_FMT"\n",
			  MAC_ARG(orig_addr));
		rtw_mesh_queue_preq(mpath, RTW_PREQ_Q_F_START | RTW_PREQ_Q_F_REFRESH);
		mpath->last_preq_to_root = rtw_get_current_time();
	}
	...
	_rtw_memcpy(mpath->rann_snd_addr, mgmt->addr2, ETH_ALEN);
	...
	if (mshcfg->dot11MeshForwarding) {
		rtw_mesh_path_sel_frame_tx(RTW_MPATH_RANN, flags, orig_addr,
...
void rtw_mesh_rx_path_sel_frame(_adapter *adapter, union recv_frame *rframe)
{
	...
	start = pframe + sizeof(struct rtw_ieee80211_hdr_3addr) + 2;

	rtw_ieee802_11_parse_elems(start, left, &elems, 1);
...
void rtw_mesh_queue_preq(struct rtw_mesh_path *mpath, u8 flags)
{
	...
	_rtw_spinlock(&mpath->state_lock);
	if (mpath->flags & RTW_MESH_PATH_REQ_QUEUED) {
	...
	_rtw_memcpy(preq_node->dst, mpath->dst, ETH_ALEN);
	preq_node->flags = flags;

	mpath->flags |= RTW_MESH_PATH_REQ_QUEUED;
	_rtw_spinunlock(&mpath->state_lock);
...
void rtw_mesh_path_start_discovery(_adapter *adapter)
{
	...
	rtw_rcu_read_lock();
	mpath = rtw_mesh_path_lookup(adapter, preq_node->dst);
	if (!mpath)
		goto enddiscovery;

	enter_critical_bh(&mpath->state_lock);
	if (mpath->flags & (RTW_MESH_PATH_DELETED | RTW_MESH_PATH_FIXED)) {
		exit_critical_bh(&mpath->state_lock);
		goto enddiscovery;
	}
	mpath->flags &= ~RTW_MESH_PATH_REQ_QUEUED;
	if (preq_node->flags & RTW_PREQ_Q_F_START) {
		if (mpath->flags & RTW_MESH_PATH_RESOLVING) {
			...
			mpath->flags &= ~RTW_MESH_PATH_RESOLVED;
			mpath->flags |= RTW_MESH_PATH_RESOLVING;
			mpath->discovery_retries = 0;
			mpath->discovery_timeout = rtw_disc_timeout_jiff(adapter);
		}
	} else if (!(mpath->flags & RTW_MESH_PATH_RESOLVING) ||
			mpath->flags & RTW_MESH_PATH_RESOLVED) {
		mpath->flags &= ~RTW_MESH_PATH_RESOLVING;
		exit_critical_bh(&mpath->state_lock);
		goto enddiscovery;
	...
	rtw_mod_timer(&mpath->timer, rtw_get_current_time() + mpath->discovery_timeout);
...
{
	struct rtw_mesh_path *mpath = (void *) ctx;
	_adapter *adapter = mpath->adapter;
	int ret;
	...
	if (mpath->flags & RTW_MESH_PATH_RESOLVED ||
			(!(mpath->flags & RTW_MESH_PATH_RESOLVING))) {
		mpath->flags &= ~(RTW_MESH_PATH_RESOLVING | RTW_MESH_PATH_RESOLVED);
		exit_critical_bh(&mpath->state_lock);
	} else if (mpath->discovery_retries < rtw_max_preq_retries(adapter)) {
		++mpath->discovery_retries;
		mpath->discovery_timeout *= 2;
		mpath->flags &= ~RTW_MESH_PATH_REQ_QUEUED;
		exit_critical_bh(&mpath->state_lock);
		rtw_mesh_queue_preq(mpath, 0);
	} else {
		mpath->flags &= ~(RTW_MESH_PATH_RESOLVING |
				  RTW_MESH_PATH_RESOLVED |
				  RTW_MESH_PATH_REQ_QUEUED);
		mpath->exp_time = rtw_get_current_time();
		exit_critical_bh(&mpath->state_lock);
		if (!mpath->is_gate && rtw_mesh_gate_num(adapter) > 0) {
			ret = rtw_mesh_path_send_to_gates(mpath);
			if (ret)
...
void rtw_mesh_path_tx_root_frame(_adapter *adapter)
{
	...
	case RTW_IEEE80211_PROACTIVE_RANN:
		rtw_mesh_path_sel_frame_tx(RTW_MPATH_RANN, flags, adapter_mac_addr(adapter),
					   ++minfo->sn, 0, NULL, 0, broadcast_addr,
...
static void rtw_update_metric_directly(_adapter *adapter)
{
	...
	for (i = 0; i < macid_ctl->num; i++) {
		...
		if (role == H2C_MSR_ROLE_MESH) {
			...
			u8 rate_idx = rtw_get_current_tx_rate(adapter, sta);
			u8 sgi = rtw_get_current_tx_sgi(adapter, sta);
			u8 bw = sta->cmn.bw_mode;
			u32 rate = rtw_desc_rate_to_bitrate(bw, rate_idx, sgi);
