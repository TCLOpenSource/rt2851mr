...
static int _rtw_bss_is_candidate_mesh_peer(WLAN_BSSID_EX *self, WLAN_BSSID_EX *target, u8 chk_chbw)
{
	int ret = 0;
	u8 *mconf_ie;
	sint mconf_ie_len;
	...
	/* allow 5,1,1 and 5,1,2 ? */
	if (chk_chbw && self->Configuration.DSConfig != target->Configuration.DSConfig)
		goto exit;
...
static int rtw_mpm_ampe_enc(_adapter *adapter, struct mesh_plink_ent *plink
	, u8* fbody, u8 *mic_ie, u8 *ampe_buf)
{
	...
	, u8 **nbuf, size_t *nlen)
{
	...
	/* encode */
	if (mic_ie) {
		int enc_ret = rtw_mpm_ampe_enc(adapter, plink, new_fbody, new_mic_ie, ampe_buf);
		if (enc_ret != _SUCCESS)
			goto exit;
	}
#endif
	...
	, struct xmit_frame **fwd_frame)
{
	...
	if (((RTW_MESH_DECACHE_BMC && is_mda_bmc)
		...
		&& rtw_mesh_decache(adapter, msa, le32_to_cpu(mctrl->seqnum))
	) {
