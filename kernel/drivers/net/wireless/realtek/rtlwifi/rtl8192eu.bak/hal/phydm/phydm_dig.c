...
)
{
	...
	if ((dig_t->pause_lv_bitmap == 0) && 
		(!(dm->support_ability & (ODM_BB_DIG | ODM_BB_FA_CNT)))) {
		
		PHYDM_DBG(dm, DBG_DIG, "Return: DIG or FA is disabled\n");
		return;
	}

	if (pause_level >= PHYDM_PAUSE_MAX_NUM) {
		PHYDM_DBG(dm, DBG_DIG, "Return: Wrong Lv\n");
		return;
	}

	PHYDM_DBG(dm, DBG_DIG, "Set pause {type, lv, IGI}={%d, %d, 0x%x}\n",
		pause_type, pause_level, igi_value);
	
	for (i = 0; i < PHYDM_PAUSE_MAX_NUM; i ++) {
		PHYDM_DBG(dm, DBG_DIG, "pause val[%d]=0x%x\n", 
										i, dig_t->pause_dig_value[i]);
	}
	...
	case PHYDM_PAUSE:
	{
		PHYDM_DBG(dm, DBG_DIG, "Pause DIG\n");
		
		dm->support_ability &= ~ODM_BB_DIG;
		
		if (dig_t->pause_lv_bitmap == 0) {
			dig_t->igi_backup = dig_t->cur_ig_value; /* Backup IGI value */
			PHYDM_DBG(dm, DBG_DIG, "Backup IGI=0x%x\n", 
														dig_t->igi_backup);
		}

		dig_t->pause_dig_value[pause_level] = igi_value; /* Record IGI value */
		dig_t->pause_lv_bitmap |= BIT(pause_level); /* Update pause level bit-map*/

		if (BIT(pause_level + 1) > dig_t->pause_lv_bitmap) {
			PHYDM_DBG(dm, DBG_DIG, "[SUCCESS ]Pause DIG\n");
			odm_write_dig(dm, igi_value);
		} else {
			PHYDM_DBG(dm, DBG_DIG, 
				"[FAIL] Pause DIG, pause_bitmap=0x%x\n", dig_t->pause_lv_bitmap);
		}
		break;
	}
	...
	case PHYDM_RESUME:
	{
		PHYDM_DBG(dm, DBG_DIG, "Resume DIG\n");
		
		/* check if the level is illegal or not */
		if ((dig_t->pause_lv_bitmap & (BIT(pause_level))) != 0) {
			
			dig_t->pause_lv_bitmap &= ~(BIT(pause_level));
			dig_t->pause_dig_value[pause_level] = 0;
		} else {
		
			PHYDM_DBG(dm, DBG_DIG, "Wrong resume Lv\n");
			break;
		}

		PHYDM_DBG(dm, DBG_DIG, "Pause_bitmap =0x%x\n", 
													dig_t->pause_lv_bitmap);

		/* Resume DIG */
		if (dig_t->pause_lv_bitmap == 0) {
			PHYDM_DBG(dm, DBG_DIG, "Revert ori_IGI\n");
			odm_write_dig(dm, dig_t->igi_backup); /* Revert IGI*/
			dig_t->is_ignore_dig = true;
			dm->support_ability |= ODM_BB_DIG;/* Enable DIG */
			break;
		}

		if (BIT(pause_level) <= dig_t->pause_lv_bitmap)
			break;

		/* Calculate the maximum level now */
		for (max_level = (pause_level - 1); max_level >= 0; max_level--) {
			if (dig_t->pause_lv_bitmap & BIT(max_level))
				break;
		}

		/* write IGI to lower level */
		PHYDM_DBG(dm, DBG_DIG, "Pause @ IGI{Lv=0x%x}=%d)\n",
			max_level, dig_t->pause_dig_value[max_level]);

		odm_write_dig(dm, dig_t->pause_dig_value[max_level]);

		break;
	}
	default:
		PHYDM_DBG(dm, DBG_DIG, "Wrong pause  type\n");
		break;
	}

	PHYDM_DBG(dm, DBG_DIG, "New pause bitmap = 0x%x\n",
		dig_t->pause_lv_bitmap);
