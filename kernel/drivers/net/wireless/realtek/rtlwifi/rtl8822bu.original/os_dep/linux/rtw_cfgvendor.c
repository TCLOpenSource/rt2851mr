...
void rtw_hal_pno_random_gen_mac_addr(PADAPTER adapter)
{
	...
	if (pwrctl == NULL) return;
	memcpy(mac_addr, pwrctl->pno_mac_addr, ETH_ALEN);
	if (mac_addr[0] == 0xFF) return;
	rtw_hal_random_gen_mac_addr(mac_addr);
	memcpy(pwrctl->pno_mac_addr, mac_addr, ETH_ALEN);
	print_hex_dump(KERN_DEBUG, "pno_mac_addr: ",
		       DUMP_PREFIX_OFFSET, 16, 1, pwrctl->pno_mac_addr,
		       ETH_ALEN, 1);
}
