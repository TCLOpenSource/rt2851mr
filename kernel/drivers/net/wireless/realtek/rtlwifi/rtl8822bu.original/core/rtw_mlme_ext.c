...
static void _mgt_dispatcher(_adapter *padapter, struct mlme_handler *ptable, union recv_frame *precv_frame)
{
	...
	if (ptable->func) {
...
#ifdef CONFIG_RTW_CFGVENDOR_RANDOM_MAC_OUI
		{
		    ...
		    if ((pwrctl == NULL) || (pwrctl->pno_mac_addr[0] == 0xFF))
			return;

		    if (!_rtw_memcmp(GetAddr1Ptr(pframe), adapter_pno_mac_addr(
			padapter), ETH_ALEN))
			return;
		}
#else
	...
	*(fctrl) = 0;
	...
	if ((pwrctl != NULL) && (pwrctl->pno_mac_addr[0] != 0xFF)
		...
		RTW_INFO("%s pno_scan_seq_num: %d\n", __func__,
			 pwrctl->pno_scan_seq_num);
		SetSeqNum(pwlanhdr, pwrctl->pno_scan_seq_num);
		pattrib->seqnum = pwrctl->pno_scan_seq_num;
		pattrib->qos_en = 1;
		pwrctl->pno_scan_seq_num++;
	} else
#endif
...
operation_by_state:
	...
	case SCAN_START:
		...
		if ((pwrctl != NULL) && (pwrctl->pno_mac_addr[0] != 0xFF)
			...
			rtw_hal_set_hw_mac_addr(padapter, pwrctl->pno_mac_addr);
			get_random_bytes(&seq_num, 2);
			pwrctl->pno_scan_seq_num = seq_num & 0xFFF;
			RTW_INFO("%s pno_scan_seq_num %d\n", __func__,
				 pwrctl->pno_scan_seq_num);
		}
#endif
