				   ...
				   u8 rf_path, u8 channel_mapped_index)
{
...
#else
	tx_pwr_idx = config_phydm_read_txagc_8822b(dm, rf_path, 0x04); /*0x04(TX_AGC_OFDM_6M)*/
#endif

	if (tx_pwr_idx >= 63)
		tx_pwr_idx = 63;

	tx_pwr_idx_offest = 63 - tx_pwr_idx;

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
	       "tx_power_index=%d tx_power_index_offest=%d rf_path=%d\n",
	       tx_pwr_idx, tx_pwr_idx_offest, rf_path);
	...
	if (method ==
		...
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		       "TXAGC(0x%x)=0x%x BBSwing(0x%x)=0x%x BBSwingIndex=%d rf_path=%d\n",
		       tmp_reg1,
