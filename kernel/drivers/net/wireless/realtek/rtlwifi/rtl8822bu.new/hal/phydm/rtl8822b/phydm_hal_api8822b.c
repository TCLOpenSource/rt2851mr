...
__iram_odm_func__
void phydm_rfe_8822b_setting(void *dm_void, u8 rfe_num, u8 path_mux_sel,
			     u8 inv_en, u8 source_sel)
{
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  ...
		  path_mux_sel, inv_en, source_sel);
...
phydm_rfe_8822b(struct dm_struct *dm, u8 channel)
{
	...
	if (rfe_type == 4 || rfe_type == 11) {
		...
		odm_set_bb_reg(dm, R_0x1990, BIT(8), 0x0);/* r_rfe_path_sel_   (RFE_CTRL_8) */
		odm_set_bb_reg(dm, R_0x1990, BIT(9), 0x0);/* r_rfe_path_sel_   (RFE_CTRL_9) */
		...
		odm_set_bb_reg(dm, R_0x1990, BIT(5), 0x0); /* r_rfe_path_sel_   (RFE_CTRL_5) */
		odm_set_bb_reg(dm, R_0x1990, BIT(10), 0x0); /* r_rfe_path_sel_   (RFE_CTRL_10) */
		odm_set_bb_reg(dm, R_0xcbc, BIT(19), 0x0); /* RFE_buffer */
...
phydm_rfe_8822b_lps(struct dm_struct *dm, boolean enable_sw_rfe)
{
	...
	} else if ((rfe_type == 1) || (rfe_type == 2) || (rfe_type == 6) || (rfe_type == 7) || (rfe_type == 9) || (rfe_type == 11)) {
		/* eFem */
		if (rf_reg18_ch <= 14) {
			/* signal source */
			if (!enable_sw_rfe) {
...
void phydm_ccapar_by_rfe_8822b(struct dm_struct *dm)
{
	...
	if (is_ifem_cca)
		if ((dm->cut_version == ODM_CUT_B && (col == 1 || col == 3) && bw_8822b == CHANNEL_WIDTH_40) ||
		    (!is_rfe_type && col == 3 && bw_8822b == CHANNEL_WIDTH_40) ||
		    (dm->rfe_type == 5 && col == 3))
	...
	if ((*dm->band_width == CHANNEL_WIDTH_20) && (((*dm->channel >= 52) && (*dm->channel <= 64)) || ((*dm->channel >= 100) && (*dm->channel <= 144))))
...
__iram_odm_func__
boolean
phydm_write_txagc_1byte_8822b(struct dm_struct *dm, u32 power_index,
...
__iram_odm_func__
u32 config_phydm_read_rf_reg_8822b(struct dm_struct *dm, enum rf_path path,
...
__iram_odm_func__
boolean
config_phydm_write_rf_reg_8822b(struct dm_struct *dm, enum rf_path path,
...
__iram_odm_func__
boolean
config_phydm_write_txagc_8822b(struct dm_struct *dm, u32 power_index,
			       enum rf_path path, u8 hw_rate)
{
	u32 offset_txagc[2] = {0x1d00, 0x1d80};
...
__iram_odm_func__
u8 config_phydm_read_txagc_8822b(struct dm_struct *dm, enum rf_path path,
				 u8 hw_rate)
{
#if (PHYDM_FW_API_FUNC_ENABLE_8822B == 1)
	u8 read_back_data;
...
__iram_odm_func__
void phydm_rxagc_switch_8822b(struct dm_struct *dm, boolean enable_rxagc_switch)
{
...
__iram_odm_func__
boolean
config_phydm_switch_agc_tab_8822b(struct dm_struct *dm, u8 channel,
				  enum agc_tab_sel tab_sel)
{
...
__iram_odm_func__
boolean
config_phydm_switch_bandwidth_8822b(struct dm_struct *dm, u8 primary_ch_idx,
				    enum channel_width bandwidth)
{
	u32 rf_reg18, val32;
	boolean rf_reg_status = true;
...
__iram_odm_func__
boolean
config_phydm_switch_channel_bw_8822b(struct dm_struct *dm, u8 central_ch,
				     u8 primary_ch_idx,
				     enum channel_width bandwidth)
{
	/* Switch band */
...
__iram_odm_func__
boolean
config_phydm_trx_mode_8822b(struct dm_struct *dm, enum bb_path tx_path,
