		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (nla_put_u32(skb, ANDR_WIFI_ATTRIBUTE_NUM_FEATURE_SET, num)) {
		err = -EINVAL;
		goto exit;
	}
	for (i = 0; i < num; i++) {
		if (nla_put_u32(skb, ANDR_WIFI_ATTRIBUTE_FEATURE_SET, reply[i])) {
			err = -EINVAL;
			goto exit;
		}
	}
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (output == NULL) {
		RTW_ERR("Allocate lstats info buffer fail!\n");
		return -ENOMEM;
	}
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (nla_put_u32(skb, LOGGER_ATTRIBUTE_RING_NUM, 1) ||
