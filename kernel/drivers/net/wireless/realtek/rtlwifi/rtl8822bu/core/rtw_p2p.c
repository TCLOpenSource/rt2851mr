...
void process_p2p_ps_ie(PADAPTER padapter, u8 *IEs, u32 IELength)
{
	...
	u8 *noa_attr; /* NoA length should be n*(13) + 2 */		//linux source code update, avoid wifi buffer overflow
	u32	attr_contentlen = 0;
	...
	while (p2p_ie) {
		...
		noa_attr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_NOA, NULL, &attr_contentlen);		//linux source code update, avoid wifi buffer overflow
		if (noa_attr) {
			find_p2p_ps = _TRUE;
			...
			if ((pwdinfo->p2p_ps_mode == P2P_PS_NONE) ||
				...
				if (attr_contentlen > 2 && (attr_contentlen - 2) % 13 == 0) {                     	//linux source code update, avoid wifi buffer overflow
