...
#endif
{
	...
	if (dm->support_ic_type & ODM_RTL8822B) {
		if ((thermal_value >= (rf->eeprom_thermal + 23)) && rf->high_ther == 0) {
			RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			       "AVG Thermal Meter = %d >= (eeprom_thermal(%d) + 23), Set 1 Tx\n",
			       thermal_value, rf->eeprom_thermal);
			config_phydm_trx_mode_8822b(dm, 1, 3, 0);
			rf->high_ther = 1;
		}
		if ((thermal_value <= (rf->eeprom_thermal + 19)) && rf->high_ther == 1) {
			RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			       "AVG Thermal Meter = %d <= (eeprom_thermal(%d) + 19), Set 2 Tx\n",
			       thermal_value, rf->eeprom_thermal);
			config_phydm_trx_mode_8822b(dm, 3, 3, 0);
			rf->high_ther = 0;
		}
	}
