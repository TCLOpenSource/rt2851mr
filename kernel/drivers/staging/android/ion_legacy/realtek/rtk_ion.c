	...
	switch(cmd)
	{
		case RT_ION_CMD_QUERY_PHY:
		{
			...
			data = (struct RT_ION_QUERY_PHY_data *)arg;
				...
				if ( IS_ERR(handle) ) {
					printk(KERN_ERR " rtIon ion_import_dma_buf fail client:%p id:0x%x  err:%d\n", client, data->handle_id, (int)PTR_ERR(handle) );
					return PTR_ERR(handle);
				}

				err = ion_phys(client, handle, (ion_phys_addr_t *)&(data->phy_addr), (size_t *)(uintptr_t *)&(data->leng) );
...
#else
//fix kasan compile error because no use
	...
	{
		{
			...
			.base = CONST_GAL_MEM_ADDR_START,
			.size = CONST_GAL_MEM_SIZE,
		},
	}
};
#endif
