...
static struct class	*bw_auth_class;
struct device		*bw_auth_dev;
#endif
extern unsigned long	cma_highmem_start;
extern unsigned long	cma_highmem_size;
extern unsigned long	cma_highmem_bw_start;
//extern unsigned long	cma_highmem_bw_size;
#endif
...
#ifdef CONFIG_HIGHMEM
#ifdef CONFIG_HIGHMEM_BW_CMA_REGION
	unsigned long bw_high_cma_size = carvedout_buf_query(CARVEDOUT_BW_HIGH_CMA, NULL);
#endif
#endif
...
#ifdef CONFIG_HIGHMEM_BW_CMA_REGION
	if (bw_high_cma_size > 0) {
		result = register_chrdev(BW_AUTH_MAJOR, BW_AUTH_NAME, &auth_fops);
		if (result < 0) {
			pr_err("bw_auth: can't get major %d...\n", BW_AUTH_MAJOR);
			return result;
		}

		bw_auth_class = class_create(THIS_MODULE, "bw_auth");
		if (IS_ERR(bw_auth_class))
			return PTR_ERR(bw_auth_class);

		bw_auth_class->devnode = auth_devnode;

		bw_auth_dev = device_create(bw_auth_class, NULL, MKDEV(BW_AUTH_MAJOR, 0), NULL, "bw_auth");
		if (cma_highmem_bw_start) {
			pr_info("[MEM] release %08lx - %08lx for high memory BW cma...\n",
				cma_highmem_bw_start, cma_highmem_bw_start + bw_high_cma_size);
			memblock_free(cma_highmem_bw_start, bw_high_cma_size);

			dma_declare_contiguous(bw_auth_dev, bw_high_cma_size, cma_highmem_bw_start, 0);
		} else {
			/* we create a null cma in order to avoid kernel from using the default cma instead */
			dma_declare_null(bw_auth_dev);
		}
	}
#endif
