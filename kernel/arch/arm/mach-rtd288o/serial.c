...
#define Get_HD_SEM_NEW_1() 		while(!rtd_inl(SB2_HD_SEM_NEW_1_reg)){;}
#define Set_HD_SEM_NEW_1() 		rtd_outl(SB2_HD_SEM_NEW_1_reg, 0);
...
int uart_setBaudrate(unsigned int baudrate)
{
    ...
    /* Set divisor latches. */
#if 1 // real ic
    if(uart_base == ISO_MISC_OFF_UART_U0RBR_THR_DLL_reg)
        divisor = ( get_uart_clock_from_reg_setting(0)/16/baudrate );
    else
        divisor = ( get_uart_clock_from_reg_setting(1)/16/baudrate );
