...
//			printk("0x%08x\n", _prom_envp[i]);
	...
	{
		printk("warning: _prom_envp is %p in prom_init\n", _prom_envp);
	}
#endif
...
/////	sprintf(platform_info.kernel_source_code_info, "%s\n%s", LINUX_SOURCE_CODE_SVN, UTS_VERSION);
	...
	if(ptr) {
		...
		strncpy(platform_info.bootloader_version, ptr, min(strlen(ptr),31));
...
static int __init early_reclaim(char *p)
{
	...
	pr_info("[POV] reclaim=0x%08llx@0x%08llx\n", reserved_dvr_size, reserved_dvr_start);
...
static int __init early_rtdlog(char *str)
{
	...
	ulong rtdlog_addr;
	int err;
	err = kstrtoul(str, 16, &rtdlog_addr);
	if(err)
		return err;
