...
int platform_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	...
	sev();
#ifdef CONFIG_REALTEK_POWER_CUT // must enable if we want to wakeup CPU1 from shutdown mode
	//if((get_platform() == PLATFORM_KXLP) || (get_platform() == PLATFOMR_KXLPP)) 
	if(0)
	{
...
#define cpu_freq_lock()
#define cpu_freq_unlock();
#endif
int _rtk_cpu_kill(unsigned int cpu)
{
	//if((get_platform() == PLATFOMR_KXLP) || (get_platform() == PLATFORM_KXLPP)) 
	{
		int cur_cpu_id;
...
int rtk_cpu_up(int cpu)
{
	//if((get_platform() == PLATFORM_KXLP) || (get_pltform() == PLATFORM_KXLPP)) 
	{
		if(cpu==1)
		{
			enable_cpu1();
