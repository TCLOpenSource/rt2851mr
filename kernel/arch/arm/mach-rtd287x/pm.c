...
static int notrace rtk_enter_suspend(unsigned long param)
{
	...
	while(1)
	{
		...
		unsigned int kcpu_resume_address = rtd_inl(STB_WDOG_DATA6_reg);
		
		if(kcpu_resume_address == 0x23792379)
		{
			//printk(KERN_ERR "KCPU STR flow not finish\n");
			if(count++ > 200) // wait 2s
				return 0;  // goto end;   // marked by kevin due to compile error
		}
		else
		{
...
{
	unsigned int regValue = rtd_inl(WKSOR_REG);
	//no need send power key to Android when wakeup source is rtc
	if(((regValue & 0xff) != 0x0) && ((regValue & 0xff)!= WKSOR_RTC))
