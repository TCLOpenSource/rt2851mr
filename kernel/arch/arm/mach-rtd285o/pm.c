...
#define EMMC_BLOCK_SIZE       (PAGE_SIZE / 8)
#define WOV_DUMP_DATA_DDR_START_ADDR		0x28000000 	//can adjust this address by different project. this is within VBM curved out range
#define WOV_AP_DDR_OFFSET			0x2000000	//this offset is used at bootcode.do not change.
#define WOV_AP_DDR_PHYSICAL_START_ADDR	(WOV_DUMP_DATA_DDR_START_ADDR-WOV_AP_DDR_OFFSET)
extern int mmc_fast_read(unsigned int, unsigned int, unsigned char *);
extern u32 wov_start_block;
extern u32 wov_size_block;


static int rtk_load_uboot_wov_ap(void)
{
	...
	for(i=0;i<count;i++){
		...
		if (mmc_fast_read(wov_start_block+i*blk, blk, (unsigned char *)tmp_ptr) != blk){
			...
			printk("load success %d\n",i);
			printk("buf_ptr= %x,tmp_ptr=%x,size=%d\n",buf_ptr+i*size,tmp_ptr,size);
...
static int rtk_pm_begin(suspend_state_t state)
{
	...
	pr_info("\npower down set freq failed\n");
    else
	pr_info("\npower down set freq successfully\n");
#endif
    need_load_av = false;
    switch (state)
    {
...
#endif
#ifdef CONFIG_REALTEK_VOLTAGE_CTRL
    if(init_cpu_freq("ondemand","/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor")!=0)
	pr_info("\npower up set freq failed\n");
    else
	pr_info("\npower up set freq successfully\n");
#endif
