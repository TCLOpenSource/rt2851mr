...
{
	if(page) {
		sscanf(page,"%hd",&platform_info.sched_log_event_number);
	}
	else
		platform_info.sched_log_event_number=100;
...
{
	char fmt[16];
	sprintf(fmt, "%%%ds", (int)(sizeof(platform_info.sched_log_name)-1));

	if(page) {
		sscanf(page,fmt,platform_info.sched_log_name);
	}
	else
...
#define POWER_ON_RESOURCE_ADDR                      (0x04eff000)     // 04eff000~04f0_0000
#define RESOURCE_TABLE_OFFSET                       (0x00080000)
...
void * __init get_platform_resource(void)
{
        ...
        {
                 unsigned long kboot_size = 0;
                 unsigned long kboot_addr = 0;
                 kboot_size = carvedout_buf_query(CARVEDOUT_K_BOOT, (void **)&kboot_addr);
                 pResourceTable = phys_to_virt(kboot_addr);
                 if(kboot_size)
                 {
                         resource_table_virt_addr = pResourceTable + rtk_resource_addr - reserved_dvr_start;
                         cal_sum = resource_table_virt_addr->magic_num + resource_table_virt_addr->scaler_resource_offset + resource_table_virt_addr->throttle_resource_offset + resource_table_virt_addr->cpu_resource_offset;
                 ...
                 {
                         printk("%s pResourceTable (0x%lx) is invalid\n", __FUNCTION__, (unsigned long)pResourceTable);
                         goto error;
                 }
        }
...
static ssize_t fw_git_version_show(struct kobject * kobj, struct kobj_attribute * attr, char * page)
{
	...
	memset(msg, 0, sizeof(msg));
	snprintf(msg, sizeof(msg)-1, LINUX_GIT_VERSION);
	strcat(page_info, msg);
	strcat(page_info, "\n");
...
static __init int early_parse_rtk_rcs (char *str)
{
    ...
    if(rtk_resource_addr != POWER_ON_RESOURCE_ADDR)                                                                                                                    
    {                                                                                                                                                                  
        rtk_resource_addr = RESOURCE_TABLE_OFFSET;                                                                                                                     
        reserved_dvr_start = 0;                                                                                                                                         
    }      
    return 0 ;
}
